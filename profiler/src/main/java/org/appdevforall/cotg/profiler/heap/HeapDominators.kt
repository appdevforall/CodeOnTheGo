...
internal object HeapDominators {
    ...
    fun fromHeapGraph(graph: HeapGraph): HeapObjectNode =
        build(
            ...
            rootIds = graph.gcRoots.asSequence()
                .map { it.id }
    ...
    private class Builder(val dom: DominatorNode) {
    ...
    private fun assemble(
        ...
        val rootDom = tree[ValueHolder.NULL_REFERENCE]
        ...
        while (queue.isNotEmpty()) {
            val id = queue.removeFirst()
            builders.getValue(id).childIds = keepChildren(builders.getValue(id).dom.dominatedObjectIds)
        }
        ...
        if (dropped) {
            logger.debug(
                "Heap dominator tree pruned to {} frames (cap {}); small subtrees fold into their parent's self.",
                discovered.size, MAX_NODES,
            )
        }
    ...
    private fun referencesOf(graph: HeapGraph, id: Long): Sequence<Long> =
    ...
    private fun shallowSizeOf(graph: HeapGraph, id: Long): Int =
        when (val obj = graph.findObjectById(id)) {
            is HeapInstance -> obj.byteSize
            is HeapObjectArray -> obj.byteSize
            is HeapPrimitiveArray -> obj.byteSize
            is HeapClass -> 0 // classes are histogrammed as roots with no shallow size (see HeapDumpAnalyzer)
        }

    private fun labelOf(graph: HeapGraph, id: Long): String =
        when (val obj = graph.findObjectById(id)) {
