...
/**
 * Persistent "back to CoGo" affordance (WS-G, complementing the 3-finger return gesture
 * in {@link QuickBuildGestures}): a small semi-transparent circular button pinned to the
 * screen's bottom-end corner, present on every activity of the test app for as long as
 * the Quick Build runtime is installed. Exists because the gesture is undiscoverable on
 * its own - the one-time text hint ({@link OverlayState#hint()}) fades after
 * {@link QuickBuildRuntime#HINT_HIDE_MS} and never comes back, leaving a user who missed
 * it with only the recents screen. Tapping runs the exact same
 * {@link QuickBuildGestures#returnToIde} path the gesture already uses.
 *
 * Deliberately unobtrusive: small (see {@link #SIZE_DP}), low-alpha until touched,
 * corner-anchored with a margin that also clears the system navigation bar (CLAUDE.md:
 * never draw over the system bars) so it never sits over an app's own controls in the
 * common case. It is its own decor-attached View, entirely outside the app's layout, so
 * it never resizes or repositions anything the app under test draws.
 */
final class ReturnToIdeButton {
	...
	private View createButton(final Activity activity) {
		...
		FrameLayout.LayoutParams params =
				new FrameLayout.LayoutParams(size, size, Gravity.BOTTOM | Gravity.END);
	...
	/**
	 * Extra bottom margin so the button clears the system navigation bar rather than
	 * sitting on top of it. The deprecated accessor is the only one available at minSdk
	 * 28 (mirrors {@code StatusOverlay#applyStatusBarInset}).
	 */
