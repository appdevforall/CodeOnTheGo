...
class OfflineNetworkGuardTest {
	...
	private static boolean containsAscii(byte[] haystack, String needle) {
		...
		outer: for (int i = 0; i <= haystack.length - pattern.length; i++) {
	...
	/**
	 * Walks up from the test's code-source location to the module `build` dir. An optional `quickbuild.offlineGuard.buildDir` system property overrides it for unusual layouts.
	 */
