...
/**
 * Builds the project-compiled supertype graph the restart closure needs, from the diverted class
 * headers. Interfaces count too, not just superclasses: a project interface with default method
 * bodies is component code, and DeployPolicy's live index counts interface edges - dropping them
 * here makes an edit to such an interface a restart-policy false negative until the component
 * class recompiles in-session.
 */
internal object SupertypeResolver {
	 ...
	 *   project-compiled subset), with unreadable entries skipped - a missing edge degrades to
