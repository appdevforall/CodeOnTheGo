...
class QuickBuildManifestTransformer(
	 ...
	 *   `<application>`, a component without android:name, or an unsupported attribute.
	 */
	...
	private fun transformActivities(
		...
		// Each renamed activity also leaves an alias under its REAL class name, or an explicit
	...
	private fun transformComponents(
		...
		var proxyIndex = 0
		return application.childElements(tag).mapIndexedNotNull { index, element ->
			val userClass = requireComponentName(element, tag, index, manifestPackage)
			rejectUnsupported(element, tag, userClass)
			unsupportedAttribute?.let { attribute ->
				if (element.getAttributeNS(ANDROID_NS, attribute) == "true") {
					throw IllegalArgumentException(
						"<$tag> '$userClass' sets android:$attribute=\"true\", which Quick Build " +
	...
	/**
	 * Fails the build on an `<application>` moving every component off the default process.
	 *
	 * The per-component check cannot see this one: android:process on `<application>` sets the
	 * default for components that do not name their own, so each component element is clean
	 * while the whole app still runs somewhere the runtime does not. That is the shape the
	 * single-process assumption is least able to survive, so it is worth its own check rather
	 * than a wider one on every element.
	 *
	 * @param application the `<application>` element; only android:process is inspected.
	 * @throws IllegalArgumentException if it declares a non-blank android:process.
	 */
	private fun rejectApplicationProcess(application: Element) {
		val process = application.getAttributeNS(ANDROID_NS, "process")
		if (process.isNotBlank()) {
			throw IllegalArgumentException(
				"<application> sets android:process=\"$process\", which moves every component off " +
					"the default process; Quick Build does not support that yet, so use a " +
					"Standard Run",
			)
		}
	}
