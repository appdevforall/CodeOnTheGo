...
class QuickBuildManifestTransformerTest {
	...
	@Test
	fun `records no authorities for a provider that declares none`() {
		val result =
			transformer().transform(
				manifest("""<provider android:name="com.example.app.DataProvider" />""").byteInputStream(),
			)

		assertThat(result.components.single { it.type == ComponentType.PROVIDER }.authorities).isEmpty()
	}

	@Test
	fun `drops empty segments when splitting a provider's authorities`() {
		// A merged manifest can carry a doubled or trailing ';' from an empty manifest
		// placeholder; an empty string in the report would read as a real, unnamed authority.
		val result =
			transformer().transform(
				manifest(
					"""<provider android:name="com.example.app.DataProvider"
						android:authorities="com.example.app.data;;com.example.app.files;" />""",
				).byteInputStream(),
			)

		assertThat(result.components.single { it.type == ComponentType.PROVIDER }.authorities)
			.containsExactly("com.example.app.data", "com.example.app.files")
			.inOrder()
	}
	...
	fun `writeTo accepts a destination that has no parent directory`() {
		...
		// be what decides whether the manifest gets written at all.
		val transformer = transformer()
		val result = transformer.transform(manifest(launcherActivity).byteInputStream())
