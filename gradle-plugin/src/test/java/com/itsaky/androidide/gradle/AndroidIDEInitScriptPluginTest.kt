...
class AndroidIDEInitScriptPluginTest {
	 ...
	 * applied by ID, which only resolves off the root buildscript classpath the init script plugin
	 * injects, so asserting ':app' proves that resolution works. The run only executes ':app:tasks',
	 * so ':nested:app' is never configured and the afterEvaluate that applies the plugin never
	 * fires - asserting it would test task-graph configuration, not plugin injection.
	 */
