...
class ProxyAppBuildRunnerEdgeTest {
	...
	fun `a message-less provisioner throw is reported by exception class name`() =
		runTest {
			...
			assertThat(result)
				.isEqualTo(ProxyAppBuildRunner.ProvisionResult.Failed(IllegalStateException::class.java.name))
		}
	...
	fun `a blocked scratch tree fails provisioning with the preparation message`() =
		runTest {
			...
			assertThat((result as ProxyAppBuildRunner.ProvisionResult.Failed).message)
				.contains("could not create its build directory")
	...
	fun `a daemon that rejects the configure fails provisioning`() =
		runTest {
			...
			assertThat(result)
				.isEqualTo(ProxyAppBuildRunner.ProvisionResult.Failed("Daemon rejected configuration"))
		}
	...
	fun `a daemon start failure carries the daemon's message`() =
		runTest {
			...
			assertThat(result).isEqualTo(ProxyAppBuildRunner.ProvisionResult.Failed("jdk missing"))
		}
	...
	fun `a message-less rebuild throw is reported by exception class name`() =
		runTest {
			...
			assertThat(result)
				.isEqualTo(ProxyAppBuildRunner.ProxyAppRebuildResult.Failed(IllegalStateException::class.java.name))
		}

	@Test
	fun `an unconfirmed reinstall passes its message through`() =
		runTest {
			provisioner.rebuildOutcome = { ProxyAppRebuildOutcome.InstallNotConfirmed("tap install to finish") }

			val result = runner().rebuildProxyApp(parkedRetry = false, superseded = { false })

			assertThat(result)
				.isEqualTo(ProxyAppBuildRunner.ProxyAppRebuildResult.InstallNotConfirmed("tap install to finish"))
		}
