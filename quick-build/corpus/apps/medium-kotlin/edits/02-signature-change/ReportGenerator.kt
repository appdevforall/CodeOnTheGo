...
class ReportGenerator(private val calculator: Calculator = MultiplicationCalculator()) {

	fun summarize(orders: List<Order>): Report {
		val lines = mutableListOf<String>()
		var factor = 1
		for (order in orders) {
			val described = calculator.compute(factor, order.items.size.coerceAtLeast(1))
			factor = described.substringAfter(":").toInt()
