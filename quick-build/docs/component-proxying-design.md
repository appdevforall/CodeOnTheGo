...
- `androidx.startup.InitializationProvider` (ADFA-4128 Bug 4): androidx's
  ...
  `NameNotFoundException -> StartupException`, crashing the test app during
  `handleBindApplication` before any activity or the quick-build runtime binds. This
  provider IS resolvable at setup-build compile time (its jar is a transitive
  dependency), so it fails at RUNTIME via self-lookup, not at setup-build compile time —
...
- `androidx.compose.ui.tooling.PreviewActivity` (ADFA-4128 Bug 7): `final` — the
  ...
  edited by user code. Blocked every Compose template's setup build.
- `androidx.profileinstaller.ProfileInstallReceiver` (ADFA-4128 Bug 7): resolves on
  some variant classpaths (it's in the merged manifest) but not on every setup build's
