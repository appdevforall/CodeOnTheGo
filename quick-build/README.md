...
> **Benchmark corpus moved out of this repo.** The corpus fixtures, harness, and all
> results now live in the standalone `CodeOnTheGo-build-benchmark` repo (formerly
> `test_app_corpus`). Every `corpus/...` path in this doc refers to that repo:
> `corpus/results/<dir>` is its `results/<dir>`, `corpus/README.md` is its
> `corpus/README.md`. The benchmark drives CoGo only through the declared interface
> (compile-daemon protocol + the flag-gated benchmark events file); see
> "Daemon protocol" below.
...
10. **The corpus lives in the benchmark repo; third-party source is never checked in
    anywhere** — synthetic apps are committed with oracles and results in
    `CodeOnTheGo-build-benchmark`; real apps are pinned by `vendor.json` and fetched
    into a gitignored cache (`corpus/README.md` there).
...
- **A SUCCESSFUL relink can still crash the app and poison the session — no self-healing
  exists** (ADFA-4128 Bug 5: the originally-reported trigger is fixed; the poisoning
  mechanism itself is confirmed NOT fixed and is broader than that one trigger). The
  first observed trigger was the old arsc-only relink dropping every file-backed
  resource — fixed by shipping the full relinked apk (see "Test-app architecture"
  above). Device-verified 2026-07-23 (`corpus/results/20260723T111950Z-bug5-verify/`)
  that a SECOND, independent, pre-existing trigger reaches the exact same poisoning
  symptom: quick-build's relink only feeds aapt2 the project's own `src/main/res`, never
  ...
  aapt2's canonical type-index assignment for every type ordered after it, so the
  baseline-compiled manifest's fixed `android:icon` resource ID resolves to the WRONG
  type against the freshly-relinked table, crashing icon inflation
  (`ClassNotFoundException: android.view.adaptive-icon`) on the very next `recreate()` —
  including one triggered by a pure code-only edit with no resource change at all. Fix
  path identified but not built: `aapt2 link --stable-ids` fed from AGP's own
  `stable_resource_ids_file` build artifact (produced for free by every setup build)
  would pin relink IDs to the baseline's, eliminating this whole class of trigger; wiring
  it through needs a new setup-build-reported field + daemon protocol field + a few
  files' worth of plumbing (gradle-plugin `QuickBuildJson`/`QuickBuildTasks`,
  `QuickBuildProjectLayout`, `DaemonProtocol`'s `RelinkRequest`, `Aapt2Link`) — a
  follow-up ticket, not a quick patch. Separately: `handlePayload` persists a resource
  payload to disk BEFORE applying it, and a crash during the activity recreate that
  follows happens outside `handlePayload`'s own try/catch (the recreate's `onCreate` runs
  on a later main-thread frame, caught only by the process's uncaught-exception handler)
  — so `failReload`'s rollback never runs, and the already-persisted table gets reapplied
  at the next process boot, repeating the same crash on every reload (even a pure code
  edit) until the session is reset. A general safety net (e.g. treating a crash during a
  pending reload as reason to distrust the just-applied resource generation and fall back
  to the last known-good one) would close this independent of any specific trigger; real
  recovery machinery, not built here, and the tracked followup.
...
- **Three named exceptions to "every component is proxied"**
  ...
  - `androidx.startup.InitializationProvider` (fixed, ADFA-4128 Bug 4) — resolvable on
    the compile classpath, so this is a RUNTIME failure, not a compile one: androidx's
    `AppInitializer` looks itself up by hardcoded component name to read its
    `<meta-data>` initializer list, and a renamed proxy breaks that self-lookup with
    `NameNotFoundException -> StartupException`, crash-looping the test app on launch
    before it can bind. Since almost every androidx library registers through this one
    merged provider (lifecycle-process, profileinstaller, emoji2, WorkManager, ...), this
    blocked every androidx-based project.
  - `androidx.compose.ui.tooling.PreviewActivity` (fixed, ADFA-4128 Bug 7) — `final`, so
    this IS the compile-time failure (b) above: debug-only Compose `@Preview` tooling,
    never a real launcher, never edited by user code. Blocked every Compose template's
    setup build.
  - `androidx.profileinstaller.ProfileInstallReceiver` (fixed, ADFA-4128 Bug 7) — this
    IS the compile-time failure (a) above, but only on SOME projects: resolves on some
    variant classpaths (it's in the merged manifest) but not on every setup build's
    proxy-compile classpath — present for the Compose template that hit this, absent
    from BasicJ's (Bug 4's repro), which declares no such receiver at all. Excluding it
...
- A deploy hands over new fds; reload = swap the payload classloader (+ a resource-table
  ...
  reload real. The resource payload is the FULL relinked resource apk (resources.arsc
  plus every compiled resource file), not a bare extracted table — a bare table cannot
  back a file-typed resource like a drawable XML or an adaptive-icon mipmap XML
  (ADFA-4128 Bug 5: a `strings.xml`-only edit crashed the reload resolving the launcher
  icon, which the edit never touched, and the broken table then persisted and poisoned
  every subsequent reload until the session was reset).
...
- `Context#getClassLoader()` is fixed to the base APK's classloader at LoadedApk-attach
  time regardless of which loader instantiated the object, so it never sees a
  payload-only class on its own - the `AppComponentFactory` picks the payload loader for
  INSTANTIATION only. Every activity proxy therefore also overrides `getClassLoader()`
  (via `QuickBuildClassLoaders.forActivity`) so by-name resolution through
  `context.getClassLoader()` - androidx `FragmentFactory` resolving a `<fragment>` tag or
  a Navigation-Component destination, `LayoutInflater` resolving a custom view - sees the
  payload loader too (ADFA-4128 Bug 9: BottomNav/Navigation-Drawer templates crashed on
  first launch with `Fragment$InstantiationException` because the default
  `FragmentFactory` resolved against the shell loader).
...
`aapt2`, `d8Jar` and `androidJar` are **optional** on `configure`. When a caller omits
any of them, the daemon discovers its own toolchain from `$ANDROID_HOME` — newest
`build-tools/<version>/aapt2` and `.../lib/d8.jar`, newest `platforms/android-<N>/
android.jar` — so an external caller (e.g. a benchmark harness) doesn't need to know
CoGo's internal toolchain layout. A field a caller still sends is used as-is (no
discovery, no wire-shape change). If a field is omitted and `ANDROID_HOME` is unset, or
the SDK doesn't have the tool, `configure` fails loudly with an `ok:false` diagnostic
naming exactly which field couldn't be resolved and why
(`ToolchainDiscovery.kt`).
...
(`DaemonResponse.PROTOCOL_VERSION`, currently `1`) so a caller can pin the version it
was written against and abort loudly on drift instead of silently misinterpreting a
changed wire shape:
