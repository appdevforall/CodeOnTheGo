 ...
 *
 * Reachable only from adb shell or root. It has to stay exported - the harness is another
 * package, and adb shell holds no START_ANY_ACTIVITY, so a non-exported activity cannot be
 * started with `am start` at all - so the manifest gates it on
 * `android.permission.DUMP`, which shell holds, root bypasses, and no third-party app can
 * obtain. The flags alone were not a gate: they are files in the public Downloads directory
 * that any app with storage access can create, which left "open a project and start a Gradle
 * build" callable by any installed app.
 *
 * Double-gated behind that (experiments AND qbbench flags), and it accepts only an existing
 * directory inside [Environment.PROJECTS_DIR], so even a shell caller can at worst open one
