		...
		private val dragHelperCallback =
			object : ViewDragHelper.Callback() {
				override fun tryCaptureView(
					child: View,
					pointerId: Int,
				): Boolean = isVerticalDragEnabled && isDownInDragHandle && child === overlappingContent
				...
				override fun getViewVerticalDragRange(child: View): Int = if (isVerticalDragEnabled) dragHeightMax else 0

				override fun getOrderedChildIndex(index: Int): Int = OVERLAPPING_CONTENT_INDEX

				override fun clampViewPositionVertical(
					child: View,
					top: Int,
					dy: Int,
				): Int = min(max(top, paddingTop), dragHeightMax)
