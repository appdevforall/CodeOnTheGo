...
class EditorProviderImpl(
	...
	override fun getOpenFiles(): List<File> = activity()?.editorViewModel?.getOpenedFiles() ?: emptyList()
	...
	override fun isFileModified(file: File): Boolean = activity()?.getEditorForFile(file)?.isModified == true

	override fun getModifiedFiles(): List<File> {
		val activity = activity() ?: return emptyList()
		return activity.editorViewModel
			.getOpenedFiles()
	...
	override fun getCurrentFileContent(): String? = inspectableEditor()?.text?.toString()
	...
	override fun getCurrentLanguageId(): String? = getCurrentFile()?.let { languageIdForFile(it) }
	...
	override fun appendToLine(
		...
		line: Int,
		text: String,
	): Boolean = lineEdit(file, line, existing = true) { insert(line, getColumnCount(line), text) }

	override fun prependToLine(
		file: File,
		line: Int,
		text: String,
	): Boolean = lineEdit(file, line, existing = true) { insert(line, 0, text) }

	override fun replaceLine(
		file: File,
		line: Int,
		newText: String,
	): Boolean = lineEdit(file, line, existing = true) { replace(line, 0, line, getColumnCount(line), newText) }
	...
	override fun clearPeerCursors(file: File) {
		onMain {
			peerPresenceOverlay.clear(file)
			true
		}
	}
