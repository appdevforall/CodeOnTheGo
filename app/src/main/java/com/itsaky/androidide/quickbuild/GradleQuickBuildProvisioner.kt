...
class GradleQuickBuildProvisioner(
	...
	companion object {
		...
		/**
		 * How long a tap waits for CoGo's Gradle sync to publish the project model before
		 * giving up. Generous on purpose: on a low-spec device a cold sync is minutes, and
		 * the alternative - failing the tap - is what made an ordinary "opened the project
		 * and tapped" read as a build failure.
		 */
