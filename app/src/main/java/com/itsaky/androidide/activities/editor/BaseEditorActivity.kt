...
abstract class BaseEditorActivity :
	...
	internal var bindingNullable: ActivityEditorBinding? = null
	val binding: ActivityEditorBinding
		get() = checkNotNull(bindingNullable) { "Activity has been destroyed" }
	...
	private val memoryUsageListener =
		MemoryUsageWatcher.MemoryUsageListener { memoryUsage ->
			memoryUsage.forEachValue { proc ->
				bindingNullable?.memUsageView?.chart?.apply {
					val dataset =
	...
	override fun bindLayout(): View {
		this.bindingNullable = ActivityEditorBinding.inflate(layoutInflater)
	...
	override fun onApplyWindowInsets(insets: WindowInsetsCompat) {
		...
		bindingNullable?.content?.bottomSheet?.setImeVisible(imeInsets.bottom > 0)
		bindingNullable?.contentCard?.updateLayoutParams<ViewGroup.LayoutParams> {
