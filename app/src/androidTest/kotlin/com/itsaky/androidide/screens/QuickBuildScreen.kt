...
object QuickBuildScreen : KScreen<QuickBuildScreen>() {
	...
	/**
	 * Asserts the Quick Build toolbar button is NOT on the toolbar - the shipping state,
	 * where the experiments flag is absent and the whole feature must be invisible.
	 *
	 * Pair it with [assertQuickBuildButtonShown] in the same test: on its own, an absence
	 * assertion also passes when the selector has rotted or the toolbar never rendered.
	 */
	fun TestContext<Unit>.assertQuickBuildButtonAbsent(timeoutMs: Long) {
		step("Editor toolbar shows no Quick Build button") {
			assertTrue(
				"Quick Build toolbar button is present with experiments off",
				quickBuildButton().waitUntilGone(timeoutMs),
			)
		}
	}
	...
	fun TestContext<Unit>.dismissQuickBuildDropdown() {
		step("Dropdown dismisses on back") {
			...
			assertTrue(
				...
				d
					.findObject(
						UiSelector().text(targetContext.getString(R.string.quick_build_menu_restart_session)),
	...
	/**
	 * Long-presses the button and taps the dropdown's "Restart session".
	 *
	 * Through the menu rather than [org.appdevforall.cotg.quickbuild.service.session.QuickBuildSessionManager.restartSessionAndReprovision]
	 * directly, because the defect T15 found was in the wiring: the menu item called the
	 * teardown-only entry point, so a working session manager still produced a dead control.
	 */
	fun TestContext<Unit>.restartSessionViaDropdown() {
		step("Long-press and choose Restart session") {
			quickBuildButton().longClick()
			val d = device.uiDevice
			val restart =
				d.findObject(
			...
			assertTrue("Restart session not shown in the dropdown", restart.waitForExists(DROPDOWN_ITEM_TIMEOUT_MS))
			restart.click()
			d.waitForIdle()
		}
	}
