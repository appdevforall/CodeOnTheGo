...
class BenchEventsFileTest {
	...
	/**
	 * Every other test injects a clock, which leaves the production default unexercised -
	 * and wallMs is what orders the harness's whole timeline, so a default stuck at a
	 * constant would silently flatten it.
	 */
	@Test
	fun `the default clock stamps real wall time`() {
		val f = fileAt()
		val before = System.currentTimeMillis()

		BenchEventsFile(f).append("session_started")

		val after = System.currentTimeMillis()
		val wallMs = JSONObject(f.readLines().single()).getLong("wallMs")
		assertThat(wallMs).isAtLeast(before)
		assertThat(wallMs).isAtMost(after)
	}
}
