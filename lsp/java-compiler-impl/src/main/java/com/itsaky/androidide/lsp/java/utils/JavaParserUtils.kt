...
object JavaParserUtils {
	...
	fun toStatement(tree: StatementTree?): Statement? {
		...
		return if (tree is VariableTree) {
			...
			StaticJavaParser.parseStatement(tree.toString())
		}
	}
	...
	fun toTypeParameter(type: TypeParameterTree): TypeParameter? = StaticJavaParser.parseTypeParameter(type.toString())
	...
	fun toMethodDeclaration(
		...
		methodDeclaration.parameters =
			IntStream
				.range(0, method.parameters.size)
				.mapToObj {
					return@mapToObj toParameter(
	...
	fun toTypeParameter(type: TypeParameterElement): TypeParameter? = StaticJavaParser.parseTypeParameter(type.toString())

	fun toTypeParameter(typeVariable: TypeVariable): TypeParameter? = StaticJavaParser.parseTypeParameter(typeVariable.toString())
...
private fun <E : Node?> Collection<E>.toNodeList(): NodeList<E?> = NodeList(this)
