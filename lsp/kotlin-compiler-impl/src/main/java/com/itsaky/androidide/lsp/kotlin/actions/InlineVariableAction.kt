...
class InlineVariableAction : BaseKotlinCodeAction() {
	...
	override suspend fun execAction(data: ActionData): InlineVariablePlan {
		...
		val env =
			// ADFA-5010 narrowed compilationEnvironmentFor to IKotlinCompilationEnvironment, the
			// interface the carrier is loaded behind; the planner needs the concrete type. Same
			// downcast the sibling ExtractVariableAction does, and a refusal if it is not that type.
			(server.compilationEnvironmentFor(nioPath) as? AbstractCompilationEnvironment)
				?: return InlineVariablePlan.refused(InlineRefusal.CouldNotAnalyse)
