...
internal class CompilationEnvironment(
	...
	override fun openFileIfNeeded(path: Path) {
		fileAnalyzer.schedule(path)
	}

	override fun onFileOpen(path: Path) {
		fileAnalyzer.schedule(path)
	}
	...
	override suspend fun onFileMoved(
		fromPath: Path,
		toPath: Path,
	) {
	...
	override fun onFileContentChanged(path: Path) {
	...
	): DiagnosticResult = context(this) { collectDiagnosticsFor(file, cancelChecker) }
