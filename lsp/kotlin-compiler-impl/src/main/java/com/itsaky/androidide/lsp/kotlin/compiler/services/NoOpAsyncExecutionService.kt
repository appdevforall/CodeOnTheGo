...
internal class NoOpAsyncExecutionService : AsyncExecutionService() {
	...
	override fun createExecutor(backgroundExecutor: Executor): ExpirableExecutor = NoOpExpirableExecutor(backgroundExecutor)
	...
	override fun <T> buildNonBlockingReadAction(callable: Callable<out T>): NonBlockingReadAction<T> = NoOpNonBlockingReadAction(callable)
}
...
private class NoOpExpirableExecutor(
...
private class NoOpNonBlockingReadAction<T>(
	...
	override fun submit(backgroundThreadExecutor: Executor): CancellablePromise<T> =
		CompletableFuture
			.supplyAsync({ callable.call() }, backgroundThreadExecutor)
			.asCancellablePromise()

	override fun executeSynchronously(): T = callable.call()
}

private fun <T> CompletableFuture<T>.asCancellablePromise(): CancellablePromise<T> = CompletableFutureCancellablePromise(this)
...
private class CompletableFutureCancellablePromise<T>(
	...
	// Promise
	override fun getState(): Promise.State =
		when {
	...
	override fun blockingGet(
		timeout: Int,
		unit: TimeUnit,
	): T = future.get(timeout.toLong(), unit)
	...
	override fun <SUB_RESULT> thenAsync(handler: Function<in T, out Promise<SUB_RESULT>>): Promise<SUB_RESULT> =
		future
			.thenCompose { value ->
