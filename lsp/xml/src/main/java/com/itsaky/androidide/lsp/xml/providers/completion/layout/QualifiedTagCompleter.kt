...
class QualifiedTagCompleter
	internal constructor(
		...
		private val lookupFactory: (ModuleProject) -> ClasspathLookup = ::classpathLookupOf,
		...
		override fun doComplete(
			...
			addFromLookup(lookupFactory(module), fqn, prefix, result)
			addFromTrie(module.compileJavaSourceClasses, fqn, prefix, result)

			return CompletionResult(result)
		}

		private fun addFromLookup(
			lookup: ClasspathLookup,
			fqn: String,
			prefix: String,
			result: MutableList<CompletionItem>,
		) {
			val packageName = if (lookup.isPackage(fqn) || lookup.isClass(fqn)) fqn else fqn.substringBeforeLast('.')

			lookup.children(packageName).forEach {
