...
class IndexWorkerBatchRemovalTest {
	...
	private fun symbol(sourceId: String) =
		JvmSymbol(
			key = "$sourceId#Type",
			sourceId = sourceId,
	...
	private fun fileMeta(path: String) =
		KtFileMetadata(
			filePath = path,
	...
	private fun removeCmd(path: String) = IndexCommand.RemoveFromIndex(Paths.get(path))

	/** A run of consecutive removals issues exactly one batched call and deletes every record. */
	@Test
	fun `N consecutive removals collapse into ONE batched removeBySources`() =
		runTest {
	...
	@Test
	fun `a non-removal command stops the batch and is pushed back unconsumed`() =
		runTest {
	...
	@Test
	fun `InMemory and SQLite-style backings give identical removeBySources results (parity)`() =
		runTest {
	...
	@Test
	fun `removeBySources only deletes the named sources`() =
		runTest {
