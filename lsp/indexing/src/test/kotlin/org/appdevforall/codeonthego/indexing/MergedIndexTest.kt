...
class MergedIndexTest {
	...
	@Test
	fun `merges results from multiple indexes`() =
		runTest {
	...
	@Test
	fun `deduplicates by key - first index wins`() =
		runTest {
	...
	@Test
	fun `respects limit across indexes`() =
		runTest {
			val idx1 =
				makeIndex().also { e ->
	...
	@Test
	fun `get returns first match in priority order`() =
		runTest {
	...
	@Test
	fun `get returns from second index when not in first`() =
		runTest {
			val idx1 = makeIndex()
	...
	@Test
	fun `get returns null if key not in any index`() =
		runTest {
	...
	@Test
	fun `containsSource checks all indexes`() =
		runTest {
	...
	@Test
	fun `distinctValues deduplicates across indexes`() =
		runTest {
			val idx1 =
				makeIndex().also { e ->
	...
	@Test
	fun `empty merged index returns empty results`() =
		runTest {
	...
	@Test
	fun `single index merged works identically to unmerged`() =
		runTest {
			val idx =
				makeIndex().also {
	...
	@Test
	fun `vararg constructor builds merged index`() =
		runTest {
