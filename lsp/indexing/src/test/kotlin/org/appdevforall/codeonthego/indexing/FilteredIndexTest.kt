...
class FilteredIndexTest {
	...
	@Test
	fun `query returns nothing when no sources active`() =
		runTest {
	...
	@Test
	fun `query returns entries for active source`() =
		runTest {
			val (_, filtered) = setupBackingAndFiltered()
	...
	@Test
	fun `deactivateSource hides entries from that source`() =
		runTest {
			val (_, filtered) = setupBackingAndFiltered()
			filtered.activateSource("src1")
	...
	@Test
	fun `setActiveSources replaces entire active set`() =
		runTest {
			val (_, filtered) = setupBackingAndFiltered()
			filtered.activateSource("src1")
	...
	@Test
	fun `get returns null when source is inactive`() =
		runTest {
			val (_, filtered) = setupBackingAndFiltered()
	...
	@Test
	fun `get returns entry when source is active`() =
		runTest {
			val (_, filtered) = setupBackingAndFiltered()
			filtered.activateSource("src1")
	...
	@Test
	fun `get returns null for nonexistent key even with active source`() =
		runTest {
			val (_, filtered) = setupBackingAndFiltered()
			filtered.activateSource("src1")
	...
	@Test
	fun `containsSource returns false when source inactive`() =
		runTest {
			val (_, filtered) = setupBackingAndFiltered()
	...
	@Test
	fun `containsSource returns true only when source is active AND in backing`() =
		runTest {
			val (_, filtered) = setupBackingAndFiltered()
			filtered.activateSource("src1")
	...
	@Test
	fun `isCached returns true if source exists in backing regardless of active state`() =
		runTest {
			val (_, filtered) = setupBackingAndFiltered()
	...
	@Test
	fun `activeSources returns current active set`() =
		runTest {
			val (_, filtered) = setupBackingAndFiltered()
			filtered.activateSource("src1")
			filtered.activateSource("src2")
	...
	@Test
	fun `isActive reflects current state`() =
		runTest {
			val (_, filtered) = setupBackingAndFiltered()
	...
	@Test
	fun `explicit sourceId query returns empty for inactive source`() =
		runTest {
			val (_, filtered) = setupBackingAndFiltered()
	...
	@Test
	fun `all sources active returns all entries from backing`() =
		runTest {
			val (_, filtered) = setupBackingAndFiltered()
			filtered.activateSource("src1")
			filtered.activateSource("src2")
	...
	@Test
	fun `close clears active sources`() =
		runTest {
			val (_, filtered) = setupBackingAndFiltered()
			filtered.activateSource("src1")
			filtered.close()
