...
class InMemoryIndexTest {
	...
	private val descriptor =
		object : IndexDescriptor<TestEntry> {
			...
			override fun fieldValues(entry: TestEntry) =
				mapOf(
					"name" to entry.name,
					"category" to entry.category,
				)

			override fun serialize(entry: TestEntry) = "${entry.key}|${entry.sourceId}|${entry.name}|${entry.category}".toByteArray()
	...
	@Test
	fun `query returns all entries when no predicates`() =
		runTest {
			val index = makeIndex()
	...
	@Test
	fun `containsSource returns true for indexed source`() =
		runTest {
			val index = makeIndex()
	...
	@Test
	fun `removeBySource removes all entries from that source`() =
		runTest {
			val index = makeIndex()
			index.insert(entry("k1", "src1", "Alpha"))
			index.insert(entry("k2", "src1", "Beta"))
	...
	@Test
	fun `insert replaces existing entry with same key`() =
		runTest {
			val index = makeIndex()
	...
	@Test
	fun `query by key returns single match`() =
		runTest {
			val index = makeIndex()
			index.insert(entry("k1", "src1", "Alpha"))
			index.insert(entry("k2", "src1", "Beta"))
	...
	@Test
	fun `query by source returns entries for that source`() =
		runTest {
			val index = makeIndex()
			index.insert(entry("k1", "src1", "Alpha"))
			index.insert(entry("k2", "src1", "Beta"))
	...
	@Test
	fun `presence filter - field exists`() =
		runTest {
			val index = makeIndex()
			index.insert(entry("k1", "src1", "Alpha", "lib"))
	...
	@Test
	fun `presence filter - field absent`() =
		runTest {
			val index = makeIndex()
			index.insert(entry("k1", "src1", "Alpha", "lib"))
			index.insert(entry("k2", "src1", "Beta"))
	...
	@Test
	fun `distinctValues returns unique field values`() =
		runTest {
			val index = makeIndex()
			index.insert(entry("k1", "src1", "Alpha", "lib"))
	...
	@Test
	fun `multi-field combined query`() =
		runTest {
			val index = makeIndex()
	...
	@Test
	fun `insertAll inserts multiple entries`() =
		runTest {
			val index = makeIndex()
	...
	@Test
	fun `removeBySource is no-op for unknown source`() =
		runTest {
			val index = makeIndex()
			index.insert(entry("k1", "src1", "Foo"))
