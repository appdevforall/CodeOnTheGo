...
class JvmLibraryIndexingServicePhaseTest {
	...
	fun `a library pass that only folds into another pass's running jobs records no phase`() {
		...
		/*
		 * Opening the FIFO blocks the first pass's scan until a writer opens it, so its job is still
		 * running when the second pass submits the same JAR. The third workspace read happens only
		 * once the second pass has released the mutex, so it marks that pass as fully submitted.
		 */
		...
		val service =
			JvmLibraryIndexingService(context, IndexingProgressTracker(), workspaceSupplier = {
	...
	private fun libraryService(vararg jars: String): JvmLibraryIndexingService {
		...
		return JvmLibraryIndexingService(context, IndexingProgressTracker(), workspaceSupplier = { workspace })
	}
