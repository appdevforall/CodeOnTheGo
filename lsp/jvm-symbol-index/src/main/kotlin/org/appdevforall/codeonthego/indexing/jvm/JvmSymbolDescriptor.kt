...
object JvmSymbolDescriptor : IndexDescriptor<JvmSymbol> {
	...
	override val fields: List<IndexField> =
		listOf(
	...
	override fun deserialize(bytes: ByteArray): JvmSymbol = fromProto(JvmSymbolData.parseFrom(bytes))
	...
	private fun classInfoToProto(d: JvmClassInfo): JvmSymbolProtos.ClassData {
		...
		d.kotlin?.let { kd ->
			builder.setKotlin(
				JvmSymbolProtos.KotlinClassData
					.newBuilder()
					.setIsData(kd.isData)
	...
	private fun functionInfoToProto(d: JvmFunctionInfo): JvmSymbolProtos.FunctionData {
		...
		d.kotlin?.let { kd ->
			builder.setKotlin(
				JvmSymbolProtos.KotlinFunctionData
					.newBuilder()
	...
	private fun fieldInfoToProto(d: JvmFieldInfo): JvmSymbolProtos.FieldData {
		...
		d.kotlin?.let { kd ->
			builder.setKotlin(
				JvmSymbolProtos.KotlinPropertyData
					.newBuilder()
					.setReceiverTypeName(kd.receiverTypeName)
					.setReceiverTypeDisplayName(kd.receiverTypeDisplayName)
	...
	private fun dataFromProto(p: JvmSymbolData): JvmSymbolInfo =
		when (p.dataCase) {
	...
	private fun kindToProto(k: JvmSymbolKind) =
		when (k) {
	...
	private fun languageToProto(l: JvmSourceLanguage) =
		when (l) {
	...
	private fun languageFromProto(l: JvmSymbolProtos.JvmSourceLanguage) =
		when (l) {
	...
	private fun visibilityToProto(v: JvmVisibility) =
		when (v) {
	...
	private fun visibilityFromProto(v: JvmSymbolProtos.JvmVisibility) =
		when (v) {
