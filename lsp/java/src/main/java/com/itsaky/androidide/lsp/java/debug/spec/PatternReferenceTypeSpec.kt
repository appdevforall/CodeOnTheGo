...
class PatternReferenceTypeSpec(
    ...
    private val stem: String = when {
        classId.startsWith('*') -> classId.substring(1)
        classId.endsWith('*') -> classId.substring(0, classId.length - 1)
        else -> classId
    }
    ...
    override fun matches(refType: ReferenceType): Boolean {
        return when {
            classId.startsWith("*") -> refType.name().endsWith(stem)
            classId.endsWith("*") -> refType.name().startsWith(stem)
            else -> refType.name().equals(classId)
        }
    }
