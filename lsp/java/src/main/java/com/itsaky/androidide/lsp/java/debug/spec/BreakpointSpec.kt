...
internal class BreakpointSpec : EventRequestSpec {
	...
	fun isSameAsDef(def: BreakpointDefinition): Boolean {
		...
		return when (def) {
			is PositionalBreakpoint -> {
				(this.data as? BreakpointData.Global)?.let { global ->
			...
			is MethodBreakpoint -> {
				...
				throw IllegalArgumentException("Unknown breakpoint type")
			}
		}
	}
	 ...
	 */
	private fun normalizeArgTypeName(
		vm: VirtualMachine,
		typeName: String,
	): String {
		/*
