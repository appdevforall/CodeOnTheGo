...
| Quick Build (experimental, ADFA-4128) | `quickbuild:core`, `quickbuild:daemon`, `quickbuild:protocol`, `quickbuild:runtime` | Build-engine-adjacent feature adding a bounded live reload path beside authoritative Gradle: session/watcher/deploy service in `quickbuild:core` (CoGo side), warm incremental compile daemon (`quickbuild:daemon`, pure JVM), and the live-reload runtime AAR in the generated proxy app (`quickbuild:runtime`, no CoGo deps). Seams into the Build engine: the proxy app build IS `gradle-plugin`'s `QuickBuildPlugin` (same init-script injection), hand-back (a finished standard build refreshing a live Quick Build session's baseline) listens to `GradleBuildService` build events, and install reuses the Run path. Nested under one `quickbuild/` group, like `lsp`, so the four runtime targets stay one unit: CoGo's process, the daemon JVM, the user's app, and the protocol they share. Boundary, glossary and module-local design decisions: `quickbuild/core/README.md`. |
