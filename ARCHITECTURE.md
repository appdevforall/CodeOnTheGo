...
| Quick Build (experimental, ADFA-4128) | `quick-build`, `quickbuild-daemon`, `quickbuild-runtime` | Build-engine-adjacent feature adding a bounded live reload path beside authoritative Gradle: session/watcher/deploy service in `quick-build` (CoGo side), warm incremental compile daemon (`quickbuild-daemon`, pure JVM), and the live-reload runtime AAR in the generated proxy app (`quickbuild-runtime`, no CoGo deps). Seams into the Build engine: the proxy app build IS `gradle-plugin`'s `QuickBuildPlugin` (same init-script injection), hand-back (a finished standard build refreshing a live Quick Build session's baseline) listens to `GradleBuildService` build events, and install reuses the Run path. Separate module group while experimental to keep blast radius small. Boundary decision: [ADR 0010](docs/adr/0010-quick-build-fast-path-boundary.md); glossary + module-local design decisions: `quick-build/README.md`. |
