...
| Quick Build (experimental, ADFA-4128) | `quickbuild:core`, `quickbuild:daemon`, `quickbuild:protocol`, `quickbuild:runtime` | Live-reloads the user's app on every save in seconds instead of a full Gradle rebuild, by running it as a generated proxy app: `core` drives the session, watcher and deploy inside CoGo, `daemon` is the on-device incremental compile JVM, `runtime` ships inside the proxy app and swaps in new code and resources, and `protocol` is the wire format they share. |
