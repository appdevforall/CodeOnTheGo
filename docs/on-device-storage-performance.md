...
The controls are what make this a filesystem finding rather than a general speedup: only the per-file I/O steps moved. Note the small app benefits by the same **45%** — this is not a large-project problem. Two apps, one edit each, n=1 before and n=2 after, A56 only: a strong signal with an independent mechanism measurement behind it, but not a distribution, and not grounds for saying "every app size". Per-step detail: `quick-build/corpus/results/20260728T172912Z-sora-deepdive/`.
