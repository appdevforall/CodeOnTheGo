...
## Spotless / formatting
- **A tripped pre-push hook costs two full Spotless runs, not one.** The hook runs `spotlessApply`, and if that *changes* anything it fails the push and leaves the tree dirty — so you commit the formatting and pay the ~4.5 min again. Run `spotlessApply` yourself before committing. `:spotlessShell` alone accounts for nearly all of that time (it walks `scripts/**`); the rest of the Spotless tasks are seconds.
