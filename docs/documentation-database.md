...
The two reader sites below open the file with `SQLiteDatabase.openDatabase(..., OPEN_READONLY)`; `PluginDocumentationManager` is the one writer, and opens `OPEN_READWRITE` to insert the rows a plugin contributes (see ADR 0001 for why raw SQLite is justified here instead of Room).
