...
- **`common/.../documentation/DocumentationRequestInterceptor.kt`** — serves the same Tier 3 paths *in-process* for WebViews wired to it (`HelpActivity` today), through `WebViewClient.shouldInterceptRequest`, so a page's assets cost a database read instead of a TCP connection each (ADFA-5176). It matches the same `http://localhost:6174/...` URL space, so nothing else changes; anything it declines — a templated page, a `/pr/` endpoint, an unknown path, a failed read — falls through to `WebServer` unchanged. Set `/sdcard/Download/CodeOnTheGo.nointercept` to force documentation back onto the server.
