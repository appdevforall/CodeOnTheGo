...
class ProtocolCodecTest {
	...
	@Test
	fun `configure without aapt2, d8Jar or androidJar parses to nulls so the daemon can self-discover them`() {
		val line = """{"id": 1, "op": "configure", "projectRoot": "/p", "classpath": [], "outDir": "/out"}"""
	...
	@Test
	fun `compile request carries an optional removedFiles list when present, empty otherwise`() {
		val withRemoved =
			ProtocolCodec.parse(
				"""{"id": 2, "op": "compile", "allSources": ["/A.kt"], "changedFiles": [], "removedFiles": ["/Gone.kt"]}""",
			)
		val withoutRemoved =
			ProtocolCodec.parse("""{"id": 3, "op": "compile", "allSources": ["/A.kt"], "changedFiles": []}""")

		assertThat(((withRemoved as ParseResult.Parsed).request as CompileRequest).removedFiles)
			.containsExactly("/Gone.kt")
		assertThat(((withoutRemoved as ParseResult.Parsed).request as CompileRequest).removedFiles)
			.isEmpty()
	}

	@Test
	fun `relink request carries an optional stableIds path when present, null otherwise`() {
		val withStableIds =
			ProtocolCodec.parse(
				"""{"id": 7, "op": "relink", "resDirs": ["/res"], "manifest": "/M.xml",
				"stableIds": "/stableIds.txt"}""".replace("\n", ""),
			)
		val withoutStableIds =
			ProtocolCodec.parse("""{"id": 8, "op": "relink", "resDirs": ["/res"], "manifest": "/M.xml"}""")

		assertThat((withStableIds as ParseResult.Parsed).request)
			.isEqualTo(RelinkRequest(7, listOf("/res"), "/M.xml", "/stableIds.txt"))
		assertThat((withoutStableIds as ParseResult.Parsed).request)
			.isEqualTo(RelinkRequest(8, listOf("/res"), "/M.xml", null))
	}

	@Test
	fun `relink request carries an optional libraryResources list when present, empty otherwise`() {
		val withLibraryResources =
			ProtocolCodec.parse(
				"""{"id": 10, "op": "relink", "resDirs": ["/res"], "manifest": "/M.xml",
				"libraryResources": ["/merged_res/values_values.arsc.flat", "/lib/drawable_x.xml.flat"]}""".replace(
					"\n",
					"",
				),
			)
		val withoutLibraryResources =
			ProtocolCodec.parse("""{"id": 11, "op": "relink", "resDirs": ["/res"], "manifest": "/M.xml"}""")

		assertThat((withLibraryResources as ParseResult.Parsed).request)
			.isEqualTo(
				RelinkRequest(
					10,
					listOf("/res"),
					"/M.xml",
					libraryResources = listOf("/merged_res/values_values.arsc.flat", "/lib/drawable_x.xml.flat"),
				),
			)
		assertThat((withoutLibraryResources as ParseResult.Parsed).request)
			.isEqualTo(RelinkRequest(11, listOf("/res"), "/M.xml"))
	}
