...
class DaemonServiceTest {
	...
	@Test
	fun `configure without aapt2, d8Jar or androidJar discovers them from ANDROID_HOME`() {
		val stdlib = TestSdk.kotlinStdlib()
		val sdkRoot = File(tempDir, "sdk")
		val buildTools = File(sdkRoot, "build-tools/35.0.0").apply { mkdirs() }
		stdlib.copyTo(File(buildTools, "aapt2"))
		File(buildTools, "lib").mkdirs()
		stdlib.copyTo(File(buildTools, "lib/d8.jar"))
		val platform = File(sdkRoot, "platforms/android-36").apply { mkdirs() }
		stdlib.copyTo(File(platform, "android.jar"))
		val discoveringService =
			DaemonService(
				log = {},
				toolchainDiscovery = ToolchainDiscovery(env = { name -> if (name == "ANDROID_HOME") sdkRoot.absolutePath else null }),
			)

		val response =
			discoveringService.configure(
				ConfigureRequest(
					id = 1,
					projectRoot = tempDir.absolutePath,
					classpath = listOf(stdlib.absolutePath),
	...
	@Test
	fun `configure without aapt2, d8Jar or androidJar fails loudly when discovery can't find them`() {
		val discoveringService = DaemonService(log = {}, toolchainDiscovery = ToolchainDiscovery(env = { null }))
		...
		assertThat(messages.all { it.contains("ANDROID_HOME") }).isTrue()
	}
}
