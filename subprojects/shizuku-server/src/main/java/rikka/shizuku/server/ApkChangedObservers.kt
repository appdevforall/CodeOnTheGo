...
object ApkChangedObservers {
	...
	fun start(
		...
		// inotify watches inode, if the there are still processes holds the file,
		// DELETE_SELF will not be triggered so we need to watch the parent folder
...
class ApkChangedObserver(
	...
	override fun onEvent(
		...
		if ((event and 0x00008000) != 0 || path == null) {
			// IN_IGNORED or path is null, ignore
			return
		}
