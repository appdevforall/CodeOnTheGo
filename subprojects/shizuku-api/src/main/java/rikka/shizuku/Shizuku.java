...
public class Shizuku {
	 ...
	 * <p>
	 * Shizuku APIs can only be used when the binder is received, or a {@link IllegalStateException} will be thrown.
	 *
	 * <p>
	 * Note:
	 * </p>
	...
	/**
	 * Same to {@link #addBinderReceivedListener(OnBinderReceivedListener)} but only call the listener immediately if the binder is already received.
	 *
	 * @param listener
	 *            OnBinderReceivedListener
	 */
	...
	/**
	 * User Service is similar to <a href="https://developer.android.com/guide/components/bound-services">Bound Services</a>. The difference is that the service runs in a different process and as the identity (Linux UID) of root (UID 0) or shell (UID 2000, if the backend is Shizuku and user starts Shizuku with adb).
	 * <p>
	 * The user service can run under "Daemon mode". Under "Daemon mode" (default behavior), the service will run forever until you call the "unbind" method. Under "Non-daemon mode", the service will be stopped when the process which called the "bind" method is dead.
	 * <p>
	 * When the "unbind" method is called, the user service will NOT be killed. You need to implement a "destroy" method in your service. The transaction code for that method is {@code 16777115} (use {@code 16777114} in aidl). In this method, you can do some cleanup jobs and call {@link System#exit(int)} in the end.
	 * <p>
	 * If the backend is Shizuku, whether in daemon mode or not, user service will be killed when Shizuku service is stopped or restarted. Shizuku sends binder to all Shizuku apps. Therefore, you only need to start the user service again.
	 * <p>
	 * <b>Use Android APIs in user service:</b>
	 * <p>
	 * There is no restrictions on non-SDK APIs in user service process. However, it is not an valid Android application process. Therefore, even you can acquire an {@code Context} instance, many APIs, such as {@code Context#registerReceiver} and {@code Context#getContentResolver} will not work. You will need to dig into Android source code to find out how things works, so that you will be able to implement your service safely and elegantly.
	 * <p>
	 * Be aware that, to let the UserService to use the latest code, "Run/Debug congfigurations" - "Always install with package manager" in Android Studio should be checked.
	 *
	 * @see UserServiceArgs
	 ...
	 *
	 * <p>
	 * For adb, context should always be <code>u:r:shell:s0</code>. <br>
	 * For root, context depends on su the user uses. E.g., context of Magisk is <code>u:r:magisk:s0</code>. If the user's su does not allow binder calls between su and app, Shizuku will switch to context <code>u:r:shell:s0</code>.
	 * </p>
	 *
	 ...
	 *             if called before binder is received
	 */
	...
	/**
	 * Similar to {@link Shizuku#bindUserService(UserServiceArgs, ServiceConnection)}, but does not start user service if it is not running.
	 *
	 * @return service version if the service is running, -1 if the service is not running. For Shizuku pre-v13, version is always 0 if service is running.
	...
	/**
	 * Remove the listener added by {@link #addBinderReceivedListener(OnBinderReceivedListener)} or {@link #addBinderReceivedListenerSticky(OnBinderReceivedListener)}.
	 *
	 * @param listener
	 *            OnBinderReceivedListener
	 * @return If the listener is removed.
	 ...
	 * Remove user service.
	 * <p>
	 * You need to implement a "destroy" method in your service, or the service will not be killed.
	 *
	 * @param remove
	 *            Remove (kill) the remote user service.
	 * @see Shizuku#bindUserService(UserServiceArgs, ServiceConnection)
	 */
	public static void unbindUserService(@NonNull UserServiceArgs args, @Nullable ServiceConnection conn, boolean remove) {
		if (remove) {
			...
			/* When unbindUserService remove=false is called, although the ShizukuServiceConnection instance is removed from ShizukuServiceConnections, it still exists (since its a Binder), and it will still receive "connected" "died" from the service, and then call the callback of its ServiceConnection connections[]. This finally leads to the ServiceConnection#onServiceConnected/onServiceDisconnected being called multiple times after bindUserService is called later, which is not expected. */

			ShizukuServiceConnection connection = ShizukuServiceConnections.get(args);

			/* For newer versions of the server, we can just call removeUserService with remove=false. This will not kill the service, but will remove the ShizukuServiceConnection instance from the server. */
	...
	public static class UserServiceArgs {
		...
		/**
		 * Daemon controls if the service should be run as daemon mode. <br>
		 * Under non-daemon mode, the service will be stopped when the app process is dead. <br>
		 * Under daemon mode, the service will run forever until {@link Shizuku#unbindUserService(UserServiceArgs, ServiceConnection, boolean)} is called.
		 * <p>
		...
		/**
		 * Set if the name suffix of the user service process. The final process name will like <code>com.example:suffix</code>.
		 *
		 ...
		 * <p>
		 * By default, user service is shared by the same packages installed in all users.
		 *
		 * @param tag
		 *            Tag
		 */
		 ...
		 * <p>
		 * Use a different version code when the service code is updated, so that the Shizuku or Sui server can recreate the user service for you.
		 *
		 * @param versionCode
		 *            Version code
		 */
		 ...
		 * <p>
		 * <a href="https://developer.android.com/distribute/best-practices/develop/64-bit">Google has required since August 2019 that all apps submitted to Google Play are 64-bit.</a>
		 * <p>
		 * <a href="https://www.arm.com/blogs/blueprint/64-bit">ARM announced that all Arm Cortex-A CPU mobile cores will be 64-bit only from 2023.</a>
		 *
		 * @param use32BitAppProcess
		 *            Use 32bit app_process
		 */
