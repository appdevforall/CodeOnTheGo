...
ggml_opt_dataset_t ggml_opt_dataset_init(
    ...
    result->buf = ggml_backend_alloc_ctx_tensors_from_buft(result->ctx,
...
void
ggml_opt_dataset_shuffle(ggml_opt_context_t opt_ctx, ggml_opt_dataset_t dataset, int64_t idata) {
    ...
    std::shuffle(dataset->permutation.begin(), dataset->permutation.begin() + ishard_max,
                 opt_ctx->rng);
}
...
void ggml_opt_dataset_get_batch(ggml_opt_dataset_t dataset, struct ggml_tensor *data_batch,
    ...
    for (int64_t ishard_batch = 0; ishard_batch < shards_per_batch; ++ishard_batch) {
        ...
        ggml_backend_tensor_set(data_batch, ptr_data, ishard_batch * dataset->nbs_data,
                                dataset->nbs_data);
        ...
        ggml_backend_tensor_set(labels_batch, ptr_labels, ishard_batch * dataset->nbs_labels,
                                dataset->nbs_labels);
    }
}

void
ggml_opt_dataset_get_batch_host(ggml_opt_dataset_t dataset, void *data_batch, size_t nb_data_batch,
...
static ggml_tensor *
map_tensor(std::map<ggml_tensor *, ggml_tensor *> &tensor_map, ggml_context *ctx,
           ggml_tensor *tensor) {
    if (!tensor) {
        return nullptr;
    }
...
static void ggml_opt_build(ggml_opt_context_t opt_ctx) {
    ...
    GGML_ASSERT((!opt_ctx->static_graphs || opt_ctx->inputs->data) &&
    ...
    if (!opt_ctx->ctx_static) {
        ...
        const size_t size_meta =
                (n_loss + tensors_per_param * n_param + tensors_const) * ggml_tensor_overhead();
        struct ggml_init_params params = {
                /*.mem_size   =*/ size_meta,
                /*.mem_buffer =*/ nullptr,
                /*.no_alloc   =*/ true,
    ...
    struct ggml_context *ctx_results = opt_ctx->static_graphs ? opt_ctx->ctx_static
                                                              : opt_ctx->ctx_compute;
    ...
    if (opt_ctx->loss_type == GGML_OPT_LOSS_TYPE_CROSS_ENTROPY) {
        ...
        opt_ctx->ncorrect = ggml_count_equal(ctx_results, opt_ctx->pred,
    ...
    if (opt_ctx->grad_accs.empty()) {
        ...
        for (int i = 0; i < n_nodes; ++i) {
            ...
            if ((accumulate && (node->flags & GGML_TENSOR_FLAG_PARAM)) ||
                ...
                opt_ctx->grad_accs[i] = ggml_new_tensor(opt_ctx->ctx_static, GGML_TYPE_F32,
        ...
        if (need_momenta && opt_ctx->build_type_alloc >= GGML_OPT_BUILD_TYPE_OPT) {
            ...
            for (int i = 0; i < n_nodes; ++i) {
                ggml_tensor *node = opt_ctx->gf->nodes[i];
                if (node->flags & GGML_TENSOR_FLAG_PARAM) {
                    opt_ctx->grad_m[i] = ggml_new_tensor(opt_ctx->ctx_static, GGML_TYPE_F32,
                                                         GGML_MAX_DIMS, node->ne);
                    opt_ctx->grad_v[i] = ggml_new_tensor(opt_ctx->ctx_static, GGML_TYPE_F32,
                                                         GGML_MAX_DIMS, node->ne);
                } else {
    ...
    if (opt_ctx->buf_static) {
        ...
        opt_ctx->buf_static = ggml_backend_alloc_ctx_tensors(opt_ctx->ctx_static,
                                                             ggml_backend_sched_get_backend(
                                                                     opt_ctx->backend_sched, 0));
    ...
    opt_ctx->gb_opt = ggml_graph_dup(opt_ctx->ctx_compute, opt_ctx->gb_grad, /*force_grads =*/
                                     true);
    ...
    opt_ctx->buf_cpu = ggml_backend_alloc_ctx_tensors_from_buft(opt_ctx->ctx_cpu,
                                                                ggml_backend_cpu_buffer_type());
}
...
ggml_opt_context_t ggml_opt_init(struct ggml_opt_params params) {
    ...
    result->gf = ggml_new_graph_custom(result->ctx_compute, GGML_DEFAULT_GRAPH_SIZE, /*grads =*/
                                       true); // Forward pass.
...
void ggml_opt_result_loss(ggml_opt_result_t result, double *loss, double *unc) {
    ...
    *loss = result->loss_per_datapoint ? mean : sum;
    ...
    const double var_sum = sum_squared / nbatches - mean *
                                                    mean; // variance without Bessel's correction, i.e. nbatches/(nbatches-1)
    *unc = result->loss_per_datapoint ? sqrt(var_sum / (nbatches - 1)) : sqrt(
            var_sum * nbatches / (nbatches - 1));
}
...
void ggml_opt_alloc(ggml_opt_context_t opt_ctx, bool backward) {
    ...
    if (opt_ctx->build_type == GGML_OPT_BUILD_TYPE_OPT && opt_ctx->opt_period > 1 &&
        opt_ctx->opt_i == 0) {
        ggml_graph_reset(opt_ctx->gb_grad);
    }
    if (backward) {
...
void ggml_opt_epoch(
    ...
    for (; ibatch < ibatch_split; ++ibatch) {
        ...
        if (callback_train) {
            callback_train(true, opt_ctx, dataset, result_train, ibatch + 1, ibatch_split,
                           t_loop_start);
        }
    }
    ...
    for (; ibatch < nbatches; ++ibatch) {
        ...
        if (callback_eval) {
            callback_eval(false, opt_ctx, dataset, result_eval, ibatch + 1 - ibatch_split,
                          nbatches - ibatch_split, t_loop_start);
        }
    }
}
...
void ggml_opt_fit(
    ...
    const int64_t ibatch_split = int64_t(((1.0f - val_split) * nbatches_logical)) *
    ...
    ggml_opt_epoch_callback epoch_callback = silent ? nullptr
    ...
    for (; epoch <= nepoch; ++epoch) {
        ...
        ggml_opt_epoch(opt_ctx, dataset, result_train, result_val, idata_split, epoch_callback,
                       epoch_callback);
        if (!silent) {
            fprintf(stderr, "\n");
        }
