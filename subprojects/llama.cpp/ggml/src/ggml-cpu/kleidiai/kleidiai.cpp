...
static const char *cpu_feature_to_string(cpu_feature f) {
    switch (f) {
        case CPU_FEATURE_NONE:
            return "NONE";
        case CPU_FEATURE_DOTPROD:
            return "DOTPROD";
        case CPU_FEATURE_I8MM:
            return "I8MM";
        case CPU_FEATURE_SVE:
            return "SVE";
        case CPU_FEATURE_SME:
            return "SME";
        default:
            return "UNKNOWN";
    }
...
namespace ggml::cpu::kleidiai {
    ...
    static void transpose_f32kxn_f16nxk(size_t n, size_t k, float *dst, const uint16_t *src,
                                        size_t rhs_stride) {
    ...
    class tensor_traits : public ggml::cpu::tensor_traits {
        bool work_size(int /* n_threads */, const struct ggml_tensor *op, size_t &size) override {
            ...
            lhs_packing_info *lhs_info = is_gemv ? &kernels->gemv_lhs_info
                                                 : &kernels->gemm_lhs_info;
        ...
        bool compute_forward_fp16(ggml_compute_params *params, struct ggml_tensor *dst) {
            ...
            const size_t lhs_packed_size = variant_call<size_t>(lhs_info->packed_size, m, k, mr, kr,
                                                                sr);
            const size_t rhs_packed_size = variant_call<size_t>(kernels->rhs_info.packed_size, n,
                                                                k);
            ...
            for (int64_t batch_idx = 0; batch_idx < batch_size; ++batch_idx) {
                const uint8_t *lhs_batch =
                        static_cast<const uint8_t *>(src1->data) + batch_idx * m * lhs_stride;
                const uint8_t *rhs_batch =
                        static_cast<const uint8_t *>(src0->data) + batch_idx * n * rhs_stride;
                ...
                // LHS packing
                {
                    ...
                    if (ith < num_threads) {
                        const int64_t num_m_per_thread0 = round_down(m_roundup_mr / num_threads,
                                                                     mr);
                        const int64_t num_m_per_threadN_1 =
                                m - (num_threads - 1) * num_m_per_thread0;

                        const int64_t m_start = ith * num_m_per_thread0;
                        const int64_t num_m_per_thread = (ith == num_threads - 1)
                                                         ? num_m_per_threadN_1 : num_m_per_thread0;

                        const size_t lhs_offset = variant_call<size_t>(kernels->gemm.get_lhs_offset,
                                                                       m_start, lhs_stride);
                        const size_t lhs_packed_offset = variant_call<size_t>(
                                lhs_info->get_packed_offset, m_start, k, mr, kr, sr);

                        const void *src_ptr = static_cast<const uint8_t *>(lhs_batch) + lhs_offset;
                        void *dst_ptr = static_cast<uint8_t *>(lhs_packed) + lhs_packed_offset;

                        variant_call<void>(lhs_info->pack_func, num_m_per_thread, k, mr, kr, sr, 0,
                                           src_ptr, lhs_stride, dst_ptr);
                    }
                }
                ...
                if (first_to_arrive.test_and_set(std::memory_order_acquire) == false) {
                    ...
                    transpose_f32kxn_f16nxk(n, k, reinterpret_cast<float *>(rhs_kxn),
                                            reinterpret_cast<const uint16_t *>(rhs_batch),
                                            rhs_stride);

                    variant_call<void>(kernels->rhs_info.pack_func, 1, n, k, nr, kr, sr,
                                       n * sizeof(float),
                ...
                // Perform the matmul
                {
                    ...
                    if (ith < num_threads) {
                        ...
                        const int64_t num_n_per_threadN_1 =
                                n - (num_threads - 1) * num_n_per_thread0;

                        const int64_t n_start = ith * num_n_per_thread0;
                        const int64_t n_to_process = (ith == num_threads - 1) ? num_n_per_threadN_1
                                                                              : num_n_per_thread0;

                        const size_t lhs_packed_offset = variant_call<size_t>(
                                kernel->get_lhs_offset, m_start, k);
                        const size_t rhs_packed_offset = variant_call<size_t>(
                                kernel->get_rhs_packed_offset, n_start, k);
                        const size_t dst_offset = kernel->get_dst_offset(m_start, n_start,
                                                                         dst_stride);
                        ...
                        variant_call<void>(kernel->run_kernel, m_to_process, n_to_process, k,
                                           lhs_ptr, rhs_ptr, dst_ptr, dst_stride, sizeof(float),
                                           -FLT_MAX, FLT_MAX);
                    }
                }
        ...
        bool compute_forward_q4_0(struct ggml_compute_params *params, struct ggml_tensor *dst) {
            ...
            if (m_start < m) {
                ...
                const float *src_ptr = reinterpret_cast<const float *>(lhs +
                                                                       lhs_info->get_offset(m_start,
                                                                                            dst->src[1]->nb[1]));
                const size_t lhs_packed_offset = variant_call<size_t>(lhs_info->get_packed_offset,
                                                                      m_start, k, QK4_0, mr, kr,
                                                                      sr);
                void *lhs_packed_ptr = static_cast<void *>(lhs_packed + lhs_packed_offset);

                variant_call<void>(lhs_info->pack_func, m_to_process, k, QK4_0, mr, kr, sr, 0,
                                   src_ptr, src_stride, lhs_packed_ptr);
            }
            ...
            const size_t dst_stride = dst->nb[1];
            const size_t lhs_packed_offset = variant_call<size_t>(lhs_info->get_packed_offset, 0, k,
                                                                  QK4_0, mr, kr, sr);
            const size_t rhs_packed_offset = variant_call<size_t>(kernel->get_rhs_packed_offset,
                                                                  n_start, k, QK4_0);
            const size_t dst_offset = kernel->get_dst_offset(0, n_start, dst_stride);
            const void *rhs_ptr = static_cast<const void *>(rhs_packed + rhs_packed_offset);
            const void *lhs_ptr = (const void *) ((const char *) lhs_packed + lhs_packed_offset);
            float *dst_ptr = reinterpret_cast<float *>(static_cast<uint8_t *>(dst->data) +
                                                       dst_offset);

            if (n_to_process > 0) {
                variant_call<void>(kernel->run_kernel, m, n_to_process, k, QK4_0, lhs_ptr, rhs_ptr,
                                   dst_ptr, dst_stride,
    ...
    public:
        int repack(struct ggml_tensor *tensor, const void *data, size_t data_size) {
            ...
            variant_call<void>(ctx.kernels->rhs_info.pack_func, 1, n, k, nr, kr, sr, QK4_0,
                               (const uint8_t *) data, nullptr, tensor->data, 0, &params);
    ...
    static ggml::cpu::tensor_traits *
    get_tensor_traits(ggml_backend_buffer_t, struct ggml_tensor *) {
        static tensor_traits traits;
        return &traits;
    }
}  // namespace ggml::cpu::kleidiai

static enum ggml_status ggml_backend_cpu_kleidiai_buffer_init_tensor(ggml_backend_buffer_t buffer,
...
static void ggml_backend_cpu_kleidiai_buffer_set_tensor(ggml_backend_buffer_t buffer,
                                                        struct ggml_tensor *tensor,
...
ggml_backend_cpu_kleidiai_buffer_type_alloc_buffer(ggml_backend_buffer_type_t buft, size_t size) {
...
static size_t ggml_backend_cpu_kleidiai_buffer_type_get_alloc_size(ggml_backend_buffer_type_t buft,
