...
static void dequantize_row_qsi4c32pscalef16(
    ...
    for (size_t b = 0; b < num_blocks; ++b) {
        uint16_t scale_f16 = *((const uint16_t *) (block_ptr +
                                                   row_in_group * num_bytes_multiplier));
        ...
        block_ptr +=
                nr_pack * num_bytes_multiplier + num_segments * nr_pack * num_bytes_per_segment;
    }
}
...
static void dequantize_row_qsi4c32ps1s0scalef16(
    ...
    const uint16_t *scales = (const uint16_t *) (packed_group + packed_row_stride -
                                                 (nr * num_blocks * num_bytes_multiplier));
...
ggml_kleidiai_select_kernels(cpu_feature cpu_features, const ggml_tensor *tensor) {
    ...
    if (tensor->op == GGML_OP_MUL_MAT && tensor->src[0] != nullptr && tensor->src[1] != nullptr) {
        for (size_t i = 0; i < NELEMS(gemm_gemv_kernels); ++i) {
            if ((cpu_features & gemm_gemv_kernels[i].required_cpu) ==
                gemm_gemv_kernels[i].required_cpu &&
