...
template<typename T>
bool gguf_read_emplace_helper(const struct gguf_reader &gr, std::vector<struct gguf_kv> &kv,
...
struct gguf_context *gguf_init_from_file_impl(FILE *file, struct gguf_init_params params) {
    ...
    if (ok && gr.read(ctx->version)) {
        ...
        */
        if (ok && (ctx->version & 0x0000FFFF) == 0x00000000) {
            GGML_LOG_ERROR(
                    "%s: failed to load model: this GGUF file version %" PRIu32 " is extremely large, is there a mismatch between the host and model endianness?\n",
                    __func__, ctx->version);
            ok = false;
        }
        ...
        if (ok && ctx->version > GGUF_VERSION) {
            GGML_LOG_ERROR(
                    "%s: this GGUF file is version %" PRIu32 " but this software only supports up to version %d\n",
    ...
    if (ok && gr.read(n_tensors)) {
        static_assert(sizeof(size_t) <= 8 && sizeof(gguf_tensor_info) >= 2,
    ...
    // KV pairs
    {
        ...
        ctx->alignment =
                alignment_idx == -1 ? GGUF_DEFAULT_ALIGNMENT : gguf_get_val_u32(ctx, alignment_idx);
    ...
    for (int64_t i = 0; ok && i < n_tensors; ++i) {
        ...
        // tensor name
        {
            ...
            try {
                ...
                GGML_LOG_ERROR(
                        "%s: encountered bad_alloc error while reading tensor name %" PRIi64 "\n",
                        __func__, i);
                ok = false;
            }
            if (name.length() >= GGML_MAX_NAME) {
                GGML_LOG_ERROR("%s: tensor name %" PRIi64 " is too long: %zu >= %d\n", __func__, i,
                               name.length(), GGML_MAX_NAME);
                ok = false;
                break;
            }
            ...
            for (int64_t j = 0; ok && j < i; ++j) {
                if (strcmp(info.t.name, ctx->info[j].t.name) == 0) {
                    GGML_LOG_ERROR(
                            "%s: duplicate tensor name '%s' for tensors %" PRIi64 " and %" PRIi64 "\n",
        ...
        // tensor shape
        {
            ...
            if (n_dims > GGML_MAX_DIMS) {
                GGML_LOG_ERROR(
                        "%s: tensor '%s' has invalid number of dimensions: %" PRIu32 " > %" PRIu32 "\n",
            ...
            for (uint32_t j = 0; ok && j < GGML_MAX_DIMS; ++j) {
                ...
                if (info.t.ne[j] < 0) {
                    GGML_LOG_ERROR(
                            "%s: tensor '%s' dimension %" PRIu32 " has invalid number of elements: %" PRIi64 " < 0\n",
            ...
            if (ok && ((INT64_MAX / info.t.ne[1] <= info.t.ne[0]) ||
                ...
                GGML_LOG_ERROR("%s: total number of elements in tensor '%s' with shape "
                               ...
                               __func__, info.t.name, info.t.ne[0], info.t.ne[1], info.t.ne[2],
                               info.t.ne[3], INT64_MAX);
                ok = false;
                break;
            }
        }
        ...
        // tensor type
        {
            ...
            if (blck_size == 0 || info.t.ne[0] % blck_size != 0) {
                GGML_LOG_ERROR("%s: tensor '%s' of type %d (%s) has %" PRId64 " elements per row, "
                               ...
                               ggml_type_name(info.t.type), info.t.ne[0], blck_size);
                ok = false;
                break;
            }
    ...
    if (params.ctx != nullptr) {
        ...
        *params.ctx = ggml_init(pdata);
        ...
        for (size_t i = 0; i < ctx->info.size(); ++i) {
            ...
            struct ggml_tensor *cur = ggml_new_tensor(ctx_data, info.t.type, GGML_MAX_DIMS,
                                                      info.t.ne);
...
void
gguf_set_arr_data(struct gguf_context *ctx, const char *key, enum gguf_type type, const void *data,
                  size_t n) {
...
void gguf_add_tensor(
    ...
    ti.offset = ctx->info.empty() ? 0 :
                ctx->info.back().offset +
                GGML_PAD(ggml_nbytes(&ctx->info.back().t), ctx->alignment);
...
void gguf_set_tensor_type(struct gguf_context *ctx, const char *name, enum ggml_type type) {
    ...
    tensor->type = type;
    GGML_ASSERT(tensor->ne[0] % blck_size == 0 &&
                "tensor row size not divisible by block size of new type");
...
struct gguf_writer_base {
    ...
    virtual void write_tensor_data(const struct gguf_tensor_info &info, size_t offset_data,
                                   size_t alignment) = 0;
...
struct gguf_writer_buf final : public gguf_writer_base {
    ...
    void write_tensor_data(const struct gguf_tensor_info &info, const size_t offset_data,
...
struct gguf_writer_file final : public gguf_writer_base {
    ...
    void write(const int8_t val) override {
        ...
        if (ret != real_val) {
            throw std::runtime_error(
                    "unexpected fputc result '" + std::to_string(ret) + "' instead of '" +
    ...
    void write(const std::vector<int8_t> &val) override {
        ...
        if (ret != val.size()) {
            throw std::runtime_error(
                    "unexpected fwrite number of bytes written, '" + std::to_string(ret) +
