...
static void llama_token_data_array_partial_sort(const llama_token_data_array &cur, int npartial,
    ...
    for (int i = 0; i < (int) cur.size; ++i) {
        ...
        int ib = int(bucket_scale * val +
                     bucket_inter); //nbuckets * (val - bucket_low) / (bucket_high - bucket_low);
...
static void
llama_token_data_array_partial_sort_inplace(llama_token_data_array *cur_p, int npartial) {
    static const auto comp = [](const llama_token_data &a, const llama_token_data &b) {
        return a.logit > b.logit;
    };
...
#pragma GCC diagnostic pop
    ...
    std::discrete_distribution<int> dist(probs_iterator{cur_p->data},
                                         probs_iterator{cur_p->data + cur_p->size});
...
llama_sampler_init(const struct llama_sampler_i *iface, llama_sampler_context_t ctx) {
...
static void
llama_sampler_greedy_apply(struct llama_sampler * /*smpl*/, llama_token_data_array *cur_p) {
    cur_p->selected = 0;
    for (size_t i = 1; i < cur_p->size; ++i) {
...
static void llama_sampler_min_p_apply(struct llama_sampler *smpl, llama_token_data_array *cur_p) {
    ...
    if (!min_p_applied) {
        ...
        const float min_logit =
                cur_p->data[0].logit + logf(ctx->p); // min logit for p_i >= p * p_max
...
static void
llama_sampler_temp_ext_apply(struct llama_sampler *smpl, llama_token_data_array *cur_p) {
...
static struct llama_sampler *llama_sampler_xtc_clone(const struct llama_sampler *smpl) {
    ...
    auto *result = llama_sampler_init_xtc(ctx->probability, ctx->threshold, ctx->min_keep,
                                          ctx->seed);
...
static void
llama_sampler_mirostat_apply(struct llama_sampler *smpl, llama_token_data_array *cur_p) {
    ...
    float k = powf((epsilon_hat * powf(2, ctx->mu)) / (1 - powf(ctx->n_vocab, -epsilon_hat)),
                   1 / s_hat);
...
static void
llama_sampler_mirostat_v2_apply(struct llama_sampler *smpl, llama_token_data_array *cur_p) {
    auto *ctx = (llama_sampler_mirostat_v2 *) smpl->ctx;

    llama_sampler_softmax_impl(cur_p, true);

    // Truncate the words with surprise values greater than mu
    cur_p->size = std::distance(cur_p->data, std::find_if(cur_p->data, cur_p->data + cur_p->size,
...
static void llama_sampler_grammar_reset(struct llama_sampler *smpl) {
    ...
    auto *grammar_new = llama_grammar_init_impl(ctx->grammar->vocab, ctx->grammar_str.c_str(),
                                                ctx->grammar_root.c_str(),
                                                ctx->grammar->lazy, trigger_patterns_c.data(),
...
static struct llama_sampler *llama_sampler_grammar_clone(const struct llama_sampler *smpl) {
    ...
    auto *result = llama_sampler_init_grammar_impl(ctx->vocab, nullptr, nullptr, false, nullptr, 0,
                                                   nullptr, 0, nullptr, 0);
    GGML_ASSERT(result);
...
struct llama_sampler *llama_sampler_init_grammar(
        ...
        const char *grammar_str,
        const char *grammar_root) {
    return llama_sampler_init_grammar_impl(vocab, grammar_str, grammar_root, /* lazy= */ false,
...
struct llama_sampler *llama_sampler_init_grammar_lazy(
        ...
        const llama_token *trigger_tokens,
        size_t num_trigger_tokens) {
    return llama_sampler_init_grammar_impl(vocab, grammar_str, grammar_root, /* lazy= */ true,
                                           trigger_words, num_trigger_words, trigger_tokens,
                                           num_trigger_tokens, nullptr, 0);
}
...
static void
llama_sampler_penalties_apply(struct llama_sampler *smpl, llama_token_data_array *cur_p) {
    auto *ctx = (llama_sampler_penalties *) smpl->ctx;

    if ((ctx->penalty_last_n == 0) ||
        (ctx->penalty_repeat == 1.0f && ctx->penalty_freq == 0.0f &&
         ctx->penalty_present == 0.0f)) {
        return;
    }
    ...
    for (size_t i = 0; i < cur_p->size; ++i) {
        ...
        cur_p->data[i].logit -=
                float(count) * ctx->penalty_freq + float(count > 0) * ctx->penalty_present;
    }
...
static void
llama_sampler_top_n_sigma_apply(struct llama_sampler *smpl, llama_token_data_array *cur_p) {
...
static void get_overlapping_token_sequences(const llama_vocab &vocab, const std::string &str,
    ...
    for (llama_token token_id = 0; token_id < (llama_token) vocab.n_tokens(); token_id++) {
        ...
        if (word.find(str) != std::string::npos) {
            ...
            while ((pos = word.find(str[0], pos + 1)) != std::string::npos) {
                ...
                if (match) {
                    std::vector<llama_token> tokenization = vocab.tokenize(str.substr(i), false,
                                                                           false);
...
static void llama_sampler_dry_apply(struct llama_sampler *smpl, llama_token_data_array *cur_p) {
    ...
    int32_t effective_dry_penalty_last_n = (ctx->dry_penalty_last_n == -1) ? ctx->total_context_size
                                                                           : std::max(
                    ctx->dry_penalty_last_n, 0);
    int last_n_repeat = std::min(
            std::min((int) ctx->last_tokens.size(), effective_dry_penalty_last_n),
            ctx->total_context_size);
        ...
        for (int k = 1; k < last_n_repeat; ++k) {
            if (k > rt) {
                ...
                while (n + k < last_n_repeat &&
                       ctx->last_tokens.rat(n) == ctx->last_tokens.rat(n + k)) {
                    ++n;
                }
                ...
                if (ctx->dry_repeat_count[last - p] < right_part_len) {
                    ...
                    while (i < last_n_repeat &&
                           ctx->last_tokens.rat(i) == ctx->last_tokens.rat(i - k)) {
                        i += 1;
                    }
...
static struct llama_sampler *llama_sampler_dry_clone(const struct llama_sampler *smpl) {
    ...
    auto *result = llama_sampler_init_dry(&dummy_vocab, ctx->total_context_size,
                                          ctx->dry_multiplier, ctx->dry_base,
                                          ctx->dry_allowed_length, ctx->dry_penalty_last_n, NULL,
                                          0);
...
struct llama_sampler *
llama_sampler_init_dry(const struct llama_vocab *vocab, int32_t n_ctx_train, float dry_multiplier,
                       float dry_base, int32_t dry_allowed_length, int32_t dry_penalty_last_n,
                       const char **seq_breakers, size_t num_breakers) {
    int32_t effective_dry_penalty_last_n = (dry_penalty_last_n == -1) ? n_ctx_train : std::max(
            dry_penalty_last_n, 0);
    std::unordered_multimap<llama_token, std::vector<llama_token>> processed_breakers;
    const int MAX_CHAR_LEN = 40;
    const int MAX_SEQ_LEN = 20;

    const bool dry_enabled = (dry_multiplier != 0.0f && dry_base >= 1.0f &&
                              dry_penalty_last_n != 0);
    ...
    if (dry_enabled && seq_breakers != nullptr && num_breakers > 0) {
        ...
        for (size_t i = 0; i < num_breakers; ++i) {
            ...
            get_overlapping_token_sequences(*vocab, sequence_break, processed_breakers,
                                            MAX_SEQ_LEN);
        }
    }
    ...
    return llama_sampler_init(
            ...
            /* .ctx   = */ new llama_sampler_dry{
                    ...
                    /* .dry_repeat_count       = */dry_enabled ? std::vector<int>(
                            effective_dry_penalty_last_n, 0) : std::vector<int>{},
                    /* .dry_max_token_repeat   = */ {},
                    /* .last_tokens            = */dry_enabled ? ring_buffer<llama_token>(
                            effective_dry_penalty_last_n) : ring_buffer<llama_token>(0),
            }
    );
}
...
struct llama_sampler *
llama_sampler_init_dry_testing(int32_t context_size, float dry_multiplier, float dry_base,
                               int32_t dry_allowed_length, int32_t dry_penalty_last_n,
                               const std::vector<std::vector<llama_token>> &seq_breakers) {
    llama_vocab dummy_vocab;
    auto *result = llama_sampler_init_dry(&dummy_vocab, context_size, dry_multiplier, dry_base,
                                          dry_allowed_length, dry_penalty_last_n, NULL, 0);
...
static void
llama_sampler_logit_bias_apply(struct llama_sampler *smpl, llama_token_data_array *cur_p) {
    ...
    for (const auto &lb: ctx->logit_bias) {
        if (lb.token >= 0 && cur_p->size > (size_t) lb.token &&
            cur_p->data[lb.token].id == lb.token) {
...
static struct llama_sampler *llama_sampler_logit_bias_clone(const struct llama_sampler *smpl) {
    ...
    return llama_sampler_init_logit_bias(ctx->n_vocab, ctx->logit_bias.size(),
                                         ctx->logit_bias.data());
}
...
struct llama_sampler *llama_sampler_init_logit_bias(
    ...
    return llama_sampler_init(
            ...
            /* .ctx   = */ new llama_sampler_logit_bias{
                    /* .n_vocab    = */ n_vocab,
                    /* .logit_bias = */std::vector<llama_logit_bias>(logit_bias,
                                                                     logit_bias + n_logit_bias),
                    /* .to_search  = */ {},
            }
    );
}
...
#define LOG_DBG_CUR(...)
    ...
    for (size_t i0 = 0; i0 < cur_p->size; ++i0) {
        for (size_t i1 = 0; i1 < cur_p->size; ++i1) {
            ...
            int len0 = ctx->vocab->token_to_piece(cur_p->data[i0].id, ctx->buf0.data(),
                                                  ctx->buf0.size(), 0, false);
            if (len0 < 0) {
                ctx->buf0.resize(len0);
                len0 = ctx->vocab->token_to_piece(cur_p->data[i0].id, ctx->buf0.data(),
                                                  ctx->buf0.size(), 0, false);
                assert(len0 > 0);
            }

            int len1 = ctx->vocab->token_to_piece(cur_p->data[i1].id, ctx->buf1.data(),
                                                  ctx->buf1.size(), 0, false);
            if (len1 < 0) {
                ctx->buf1.resize(len1);
                len1 = ctx->vocab->token_to_piece(cur_p->data[i1].id, ctx->buf1.data(),
                                                  ctx->buf1.size(), 0, false);
                assert(len1 > 0);
            }
...
void llama_perf_sampler_print(const struct llama_sampler *chain) {
    ...
    LLAMA_LOG_INFO(
            ...
            __func__, data.t_sample_ms, data.n_sample, data.t_sample_ms / data.n_sample,
            1e3 / data.t_sample_ms * data.n_sample);
}
