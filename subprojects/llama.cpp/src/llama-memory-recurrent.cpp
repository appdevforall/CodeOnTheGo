...
llama_memory_recurrent::llama_memory_recurrent(
...

    {
        ...
        LLAMA_LOG_INFO(
                "%s: size = %7.2f MiB (%6u cells, %3d layers, %2u seqs), R (%s): %7.2f MiB, S (%s): %7.2f MiB\n",
                __func__,
                (float) (memory_size_r + memory_size_s) / (1024.0f * 1024.0f), mem_size, n_layer,
                n_seq_max,
...
void llama_memory_recurrent::seq_cp(llama_seq_id seq_id_src, llama_seq_id seq_id_dst, llama_pos p0,
                                    llama_pos p1) {
...
void
llama_memory_recurrent::seq_add(llama_seq_id seq_id, llama_pos p0, llama_pos p1, llama_pos shift) {
    if (shift == 0) {
        return;
    }
...
llama_memory_recurrent::init_batch(llama_batch_allocr &balloc, uint32_t n_ubatch, bool embd_all) {
    do {
        balloc.split_reset();
...
bool llama_memory_recurrent::find_slot(const llama_ubatch &ubatch) {
    ...
    for (uint32_t s = 0; s < n_seqs; ++s) {
        ...
        for (uint32_t j = 0; j < n_seq_id; ++j) {
            ...
            if (seq_id < 0 || (uint32_t) seq_id >= size) {
                ...
                LLAMA_LOG_ERROR(
                        "%s: seq_id=%d >= n_seq_max=%u Try using a bigger --parallel value\n",
    ...
    for (uint32_t s = 0; s < n_seqs; ++s) {
        ...
        if (cell.pos >= 0 && last_pos != cell.pos + (llama_pos) n_seq_tokens) {
            ...
            LLAMA_LOG_WARN(
                    "%s: non-consecutive token position %d after %d for sequence %d with %u new tokens\n",
...
void llama_memory_recurrent::state_write(llama_io_write_i &io, llama_seq_id seq_id,
...
bool llama_memory_recurrent::state_read_meta(llama_io_read_i &io, uint32_t cell_count,
    ...
    for (uint32_t il = 0; il < n_layer; ++il) {
        ...
        if (r_size_row != r_size_row_ref) {
            LLAMA_LOG_ERROR("%s: mismatched r row size (%zu != %zu, layer %d)\n", __func__,
                            r_size_row, (size_t) r_size_row_ref, il);
            return false;
        }

        if (cell_count) {
            // Read and set the keys for the whole cell range
            ggml_backend_tensor_set(r_l[il], io.read(cell_count * r_size_row), head * r_size_row,
                                    cell_count * r_size_row);
        }
    }
        ...
        for (uint32_t il = 0; il < n_layer; ++il) {
            ...
            if (s_size_row != s_size_row_ref) {
                LLAMA_LOG_ERROR("%s: mismatched s row size (%zu != %zu, layer %d)\n", __func__,
                                s_size_row, (size_t) s_size_row_ref, il);
                return false;
            }

            if (cell_count) {
                // Read and set the values for the whole cell range
                ggml_backend_tensor_set(s_l[il], io.read(cell_count * s_size_row),
                                        head * s_size_row, cell_count * s_size_row);
            }
        }
    } else {
        ...
        for (uint32_t il = 0; il < n_layer; ++il) {
            ...
            if (s_size_el != s_size_el_ref) {
                LLAMA_LOG_ERROR("%s: mismatched s element size (%zu != %zu, layer %d)\n", __func__,
                                s_size_el, (size_t) s_size_el_ref, il);
                return false;
            }
            ...
            if (cell_count) {
                ...
                for (uint32_t j = 0; j < n_embd_s; ++j) {
                    ...
                    ggml_backend_tensor_set(s_l[il], io.read(cell_count * s_size_el), dst_offset,
                                            cell_count * s_size_el);
                }
            }
        }
    }
...
llama_memory_recurrent_context::llama_memory_recurrent_context(llama_memory_status status) : status(
        status) {}
...
llama_memory_recurrent_context::llama_memory_recurrent_context(
        ...
        std::vector<llama_ubatch> ubatches) : status(LLAMA_MEMORY_STATUS_SUCCESS), mem(mem),
