...
llama_kv_cache_iswa::llama_kv_cache_iswa(
    ...
    uint32_t size_swa = std::min(size_base,
                                 GGML_PAD(hparams.n_swa * (unified ? n_seq_max : 1) + n_ubatch,
                                          n_pad));
...
void llama_kv_cache_iswa::seq_cp(llama_seq_id seq_id_src, llama_seq_id seq_id_dst, llama_pos p0,
                                 llama_pos p1) {
...
void
llama_kv_cache_iswa::seq_add(llama_seq_id seq_id, llama_pos p0, llama_pos p1, llama_pos shift) {
...
llama_kv_cache_iswa::init_batch(llama_batch_allocr &balloc, uint32_t n_ubatch, bool embd_all) {
...
void llama_kv_cache_iswa::state_write(llama_io_write_i &io, llama_seq_id seq_id,
...
void llama_kv_cache_iswa::state_read(llama_io_read_i &io, llama_seq_id seq_id,
...
llama_kv_cache_iswa_context::llama_kv_cache_iswa_context(llama_memory_status status) : status(
        status) {}
