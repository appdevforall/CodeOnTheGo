...
bool llama_adapter_cvec::apply(
    ...
    for (size_t il = 1; il < hparams.n_layer; il++) {
        ...
        const size_t off =
                n_embd * (il - 1); // buffer doesn't have data for layer 0, since it's never present
        if (off + n_embd <= len) {
            ggml_backend_tensor_set(tensors[il], data + off, 0,
...
static void llama_adapter_lora_init_impl(llama_model &model, const char *path_lora,
        ...
        for (int i = 0; i < gguf_get_n_kv(gguf_ctx); i++) {
            ...
            const std::string type_name =
                    ...
                    ? format("%s[%s,%zu]", gguf_type_name(type),
                             gguf_type_name(gguf_get_arr_type(gguf_ctx, i)),
                             gguf_get_arr_n(gguf_ctx, i))
                    : gguf_type_name(type);
            ...
            std::string print_value = value.size() > MAX_VALUE_LEN ? format("%s...", value.substr(0,
                                                                                                  MAX_VALUE_LEN -
                                                                                                  3).c_str())
                                                                   : value;
    ...
    for (ggml_tensor *cur = ggml_get_first_tensor(ctx.get()); cur; cur = ggml_get_next_tensor(
            ctx.get(), cur)) {
    ...
    for (auto &it: ab_map) {
        ...
        if (!model_tensor) {
            throw std::runtime_error("LoRA tensor '" + name +
    ...
    // set tensor data
    {
        ...
        auto set_tensor = [&](ggml_tensor *orig, ggml_tensor *dev) {
            size_t offs = gguf_get_data_offset(ctx_gguf.get()) +
                          gguf_get_tensor_offset(ctx_gguf.get(),
                                                 gguf_find_tensor(ctx_gguf.get(), orig->name));
...
int32_t llama_adapter_meta_val_str(const llama_adapter_lora *adapter, const char *key, char *buf,
                                   size_t buf_size) {
...
int32_t llama_adapter_meta_key_by_index(const llama_adapter_lora *adapter, int i, char *buf,
                                        size_t buf_size) {
...
int32_t llama_adapter_meta_val_str_by_index(const llama_adapter_lora *adapter, int32_t i, char *buf,
                                            size_t buf_size) {
    if (i < 0 || i >= (int) adapter->gguf_kv.size()) {
        if (buf_size > 0) {
            buf[0] = '\0';
