...
const char *llama_grammar_parser::parse_sequence(
    ...
    auto handle_repetitions = [&](int min_times, int max_times) {
        ...
        for (int i = 0; i < n_opt; i++) {
            ...
            if (i > 0 || max_times < 0) {
                rec_rule.push_back(
                        {LLAMA_GRETYPE_RULE_REF, max_times < 0 ? rec_rule_id : last_rec_rule_id});
            }
...
static bool llama_grammar_detect_left_recursion(
    ...
    for (size_t i = 0; i < rule.size(); i++) {
        if (rule[i].type == LLAMA_GRETYPE_RULE_REF && recurse_into_nonterminal) {
            if (llama_grammar_detect_left_recursion(rules, (size_t) rule[i].value, rules_visited,
                                                    rules_in_progress, rules_may_be_empty)) {
                return true;
            }
...
struct llama_grammar *llama_grammar_init_impl(
    ...
    for (size_t i = 0; i < n_rules; i++) {
        ...
        if (llama_grammar_detect_left_recursion(vec_rules, i, &rules_visited, &rules_in_progress,
                                                &rules_may_be_empty)) {
            LLAMA_LOG_ERROR(
                    "unsupported grammar, left recursion detected for nonterminal at index %zu", i);
            return nullptr;
        }
    }
...
void llama_grammar_apply_impl(const struct llama_grammar &grammar, llama_token_data_array *cur_p) {
    ...
    const auto rejects = llama_grammar_reject_candidates(grammar.rules, grammar.stacks,
                                                         candidates_grammar);
...
void llama_grammar_accept_impl(struct llama_grammar &grammar, llama_token token) {
    ...
    if (grammar.awaiting_trigger) {
        if (std::find(grammar.trigger_tokens.begin(), grammar.trigger_tokens.end(), token) !=
