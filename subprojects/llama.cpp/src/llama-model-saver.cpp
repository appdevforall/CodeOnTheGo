...
llama_model_saver::llama_model_saver(const struct llama_model &model) : model(model),
                                                                        llm_kv(model.arch) {
...
void llama_model_saver::add_kv(const enum llm_kv key, const char value) {
    ...
    GGML_UNUSED(value);
    GGML_ABORT(
            "fatal error"); // this should never be called, only needed to make the template below compile
}

template<typename Container>
void
llama_model_saver::add_kv(const enum llm_kv key, const Container &value, const bool per_layer) {
    ...
    if (std::is_same<typename Container::value_type, uint8_t>::value) {
        ...
        gguf_set_val_str(gguf_ctx, llm_kv(key).c_str(),
...
void llama_model_saver::add_kv_from_model() {
    ...
    for (int32_t id = 0; id < n_vocab; ++id) {
        ...
        switch (token_data.attr) {
            ...
            case LLAMA_TOKEN_ATTR_CONTROL:
                ...
                break;
            case LLAMA_TOKEN_ATTR_USER_DEFINED:
    ...
    const float rope_scaling_factor =
            hparams.rope_freq_scale_train == 1.0f ? 0.0f : 1.0f / hparams.rope_freq_scale_train;
