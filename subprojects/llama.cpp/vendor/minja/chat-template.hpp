...
namespace minja {
    ...
    public:
        ...
        chat_template(const std::string &source, const std::string &bos_token,
            ...
            const json dummy_str_user_msg = {{"role",    "user"},
                                             {"content", user_needle}};
            ...
            caps_.requires_typed_content =
                    !contains(try_raw_render(json::array({dummy_str_user_msg}), {}, false),
                              user_needle)
                    && contains(try_raw_render(json::array({dummy_typed_user_msg}), {}, false),
                                user_needle);
            ...
            const json needle_system_msg = {
                    {"role",    "system"},
                    {"content", caps_.requires_typed_content ? json::array(
                            {{{"type", "text"}, {"text", sys_needle}}}) : json(sys_needle)},
            };

            caps_.supports_system_role = contains(
                    try_raw_render({needle_system_msg, dummy_user_msg,}, {}, false), sys_needle);
            ...
            const auto render_with_content = [&](const json &content) {
                ...
                return try_raw_render(
                        json::array({dummy_user_msg, assistant_msg, dummy_user_msg, assistant_msg}),
                        {}, false);
            };
            auto out_empty = render_with_content("");
            auto out_null = render_with_content(json());
            caps_.requires_non_null_content =
                    contains(out_empty, user_needle) && !contains(out_null, user_needle);
            ...
            out = try_raw_render(json::array({
                                                     dummy_user_msg,
                                                     make_tool_calls_msg(json::array(
                                                             {make_tool_call("ipython",
                                                                             dummy_args_obj.dump())})),
                                             }), {}, false);
            auto tool_call_renders_str_arguments = contains(out, "<parameter=argument_needle>") ||
                                                   contains(out, "\"argument_needle\":") ||
                                                   contains(out, "'argument_needle':");
            out = try_raw_render(json::array({
                                                     dummy_user_msg,
                                                     make_tool_calls_msg(json::array(
            ...
            auto tool_call_renders_obj_arguments = contains(out, "<parameter=argument_needle>") ||
                                                   contains(out, "\"argument_needle\":") ||
                                                   contains(out, "'argument_needle':");
            ...
            if (caps_.supports_tool_calls) {
                auto dummy_args = caps_.requires_object_arguments ? dummy_args_obj : json(
                        dummy_args_obj.dump());
        ...
        std::string apply(
            ...
            auto polyfill_tool_responses = opts.polyfill_tool_responses && has_tool_responses &&
                                           !caps_.supports_tool_responses;
            auto polyfill_object_arguments = opts.polyfill_object_arguments && has_tool_calls &&
                                             caps_.requires_object_arguments;
            auto polyfill_typed_content = opts.polyfill_typed_content && has_string_content &&
                                          caps_.requires_typed_content;
            ...
            if (needs_polyfills) {
                ...
                auto add_message = [&](const json &msg) {
                    if (polyfill_typed_content && msg.contains("content") &&
                        !msg.at("content").is_null() && msg.at("content").is_string()) {
                ...
                if (polyfill_tools) {
                    adjusted_messages = add_system(inputs.messages,
                                                   ...
                                                   minja::Value(inputs.tools).dump(2, /* to_json= */
                                                                                   true) +
                                                   (!polyfill_tool_call_example ||
                                                    tool_call_example_.empty() ? "" :
                ...
                for (const auto &message_: adjusted_messages) {
                    ...
                    if (!message.contains("role") ||
                        (!message.contains("content") && !message.contains("tool_calls"))) {
                        throw std::runtime_error(
                                "message must have 'role' and one of 'content' or 'tool_calls' fields: " +
                                message.dump());
                    }
                    ...
                    if (!message["content"].is_null() && polyfill_system_role) {
                        ...
                        if (role == "system") {
                            ...
                            if (role == "user") {
                                if (!pending_system.empty()) {
                                    message["content"] = pending_system +
                                                         (content.empty() ? "" : "\n" + content);
                                    pending_system.clear();
                                }
                            } else {
            ...
            if (opts.define_strftime_now) {
                auto now = inputs.now;
                context->set("strftime_now", Value::callable(
                        [now](const std::shared_ptr<minja::Context> &,
        ...
        add_system(const nlohmann::ordered_json &messages, const std::string &system_prompt) {
