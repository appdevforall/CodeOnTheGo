...
static std::string
format_time(const std::chrono::system_clock::time_point &now, const std::string &format) {
...
common_chat_msg_diff::compute_diffs(const common_chat_msg &previous_msg,
                                    const common_chat_msg &new_msg) {
    std::vector<common_chat_msg_diff> diffs;
    if (previous_msg.reasoning_content != new_msg.reasoning_content) {
        auto &diff = diffs.emplace_back();
        diff.reasoning_content_delta = string_diff(previous_msg.reasoning_content,
    ...
    try {
        ...
        for (const auto &message: messages) {
            if (!message.is_object()) {
                throw std::runtime_error(
                        "Expected 'message' to be an object, got " + message.dump());
            }
            ...
            if (has_content) {
                ...
                if (content.is_string()) {
                    ...
                    for (const auto &part: content) {
                        ...
                        if (type != "text") {
                            throw std::runtime_error(
                                    "Unsupported content part type: " + type.dump());
                        }
                    ...
                    throw std::runtime_error(
                            "Invalid 'content' type: expected string or array, got " +
                            content.dump() +
...
std::vector<common_chat_tool> common_chat_tools_parse_oaicompat(const json &tools) {
    ...
    try {
        ...
        throw std::runtime_error(
                "Failed to parse tools: " + std::string(e.what()) + "; tools = " + tools.dump(2));
    }
...
const char *
common_chat_templates_source(const struct common_chat_templates *tmpls, const char *variant) {
    if (variant != nullptr) {
...
common_chat_templates_ptr common_chat_templates_init(
    ...
    if (model) {
        ...
        const auto get_token = [&](llama_token token, const char *name,
                                   const char *jinja_variable_name) {
    ...
    try {
        tmpls->template_default = std::make_unique<minja::chat_template>(default_template_src,
                                                                         token_bos, token_eos);
    } catch (const std::exception &e) {
        LOG_ERR("%s: failed to parse chat template (defaulting to chatml): %s \n", __func__,
                e.what());
        tmpls->template_default = std::make_unique<minja::chat_template>(CHATML_TEMPLATE_SRC,
                                                                         token_bos, token_eos);
    }
    if (!template_tool_use_src.empty()) {
        try {
            tmpls->template_tool_use = std::make_unique<minja::chat_template>(template_tool_use_src,
                                                                              token_bos, token_eos);
        } catch (const std::exception &e) {
...
static std::string
wrap_code_as_arguments(common_chat_msg_parser &builder, const std::string &code) {
...
static void parse_json_tool_calls(
        ...
        const std::function<std::string(
                const common_chat_msg_parser::find_regex_result &fres)> &get_function_name = nullptr) {
            ...
            if (res) {
                ...
                if (builder.input()[builder.pos()] == '{' || !maybe_raw_python) {
                    if (auto arguments = builder.try_consume_json_with_dumped_args({{}})) {
                        if (!builder.add_tool_call(name, "", arguments->value) ||
                            arguments->is_partial) {
...
static void
parse_prefixed_json_tool_call_array(common_chat_msg_parser &builder, const common_regex &prefix,
...
static common_chat_params common_chat_params_init_generic(const common_chat_template &tmpl,
    ...
    const auto tool_call =
            ...
            ? json{
                    ...
                    {"properties", {
                                           {"tool_calls", {
                                                                  {"type", "array"},
                                                                  {"items",
                                                                          tool_call_schemas.size() ==
                                                                          1 ? tool_call_schemas[0]
                                                                            : json{
                                                                                  {"anyOf",
                                                                                   tool_call_schemas},
                                                                          }},
            ...
            : json{
                    {"type",       "object"},
                    {"properties", {
                                           {"tool_call", tool_call_schemas.size() == 1
                                                         ? tool_call_schemas[0] : json{
...
static void common_chat_parse_generic(common_chat_msg_parser &builder) {
    ...
    if (data.value.contains("tool_calls")) {
        ...
        builder.add_content(
                response.is_string() ? response.template get<std::string>() : response.dump(2));
        if (data.is_partial) {
            throw common_chat_msg_partial_exception("incomplete response");
        }
    } else {
        throw common_chat_msg_partial_exception(
                "Expected 'tool_call', 'tool_calls' or 'response' in JSON");
    }
}
...
static common_chat_params common_chat_params_init_command_r7b(const common_chat_template &tmpl,
    ...
    for (const auto &msg: inputs.messages) {
        auto has_reasoning_content =
                msg.contains("reasoning_content") && msg.at("reasoning_content").is_string();
...
static void expect_tool_parameters(const std::string &name, const json &parameters,
    ...
    if (!parameters.is_object() || !parameters.contains("type") ||
        parameters.at("type") != "object" || !parameters.contains("properties") ||
        !parameters.contains("required")) {
        throw std::runtime_error(
                "Parameters of tool " + name + " must be an object w/ required properties");
    }
    ...
    for (const auto &prop: expected_properties) {
        if (!parameters_properties.contains(prop)) {
            throw std::runtime_error(
                    "Parameters of tool " + name + " is missing property: " + prop); // NOLINT
        }
        if (std::find(parameters_required.begin(), parameters_required.end(), json(prop)) ==
            parameters_required.end()) {
            throw std::runtime_error(
                    "Parameters of tool " + name + " must have property marked as required: " +
                    prop); // NOLINT
        }
    }
    if (parameters_properties.size() != expected_properties.size()) {
        throw std::runtime_error(
                "Parameters of tool " + name + " must only have these properties:" +
                string_join(expected_properties, ", "));
    }
}
...
static common_chat_params common_chat_params_init_llama_3_x(const common_chat_template &tmpl,
        ...
        data.grammar = build_grammar([&](const common_grammar_builder &builder) {
            ...
            auto handle_builtin_tool = [&](const std::string &name, const json &parameters) {
                ...
                for (const auto &[key, value]: parameters.at("properties").items()) {
                    kvs.push_back("\"" + key + "=\" " +
                                  builder.add_schema(name + "-args-" + key, value)); // NOLINT
                }
            ...
            if (!builtin_tools.empty()) {
                data.grammar_triggers.push_back(
                        {COMMON_GRAMMAR_TRIGGER_TYPE_WORD, "<|python_tag|>"});
    ...
    data.prompt = apply(tmpl, inputs, /* messages_override =*/ std::nullopt, /* tools_override= */
                        std::nullopt, json{
...
static void
common_chat_parse_llama_3_1(common_chat_msg_parser &builder, bool with_builtin_tools = false) {
    if (!builder.syntax().parse_tool_calls) {
        builder.add_content(builder.consume_rest());
        return;
    }
...
static void common_chat_parse_deepseek_v3_1(common_chat_msg_parser &builder) {
    ...
    LOG_DBG("%s: thinking_forced_open: %s\n", __func__,
            std::to_string(builder.syntax().thinking_forced_open).c_str());
...
static common_chat_params common_chat_params_init_gpt_oss(const common_chat_template &tmpl,
    ...
    if (inputs.tools.is_array() && !inputs.tools.empty()) {
        ...
        data.grammar = build_grammar([&](const common_grammar_builder &builder) {
            ...
            auto channel = builder.add_rule("channel",
            ...
            auto recipient_in_channel = builder.add_rule("recipient_in_channel",
                                                         ...
                                                         string_join(
                                                                 tool_rules_recipient_in_channel,
                                                                 " | ") + " )"
            );
...
static void common_chat_parse_gpt_oss(common_chat_msg_parser &builder) {
    ...
    static const common_regex tool_call1_regex(
            recipient + "<\\|channel\\|>(analysis|commentary)" + constraint + "?");
    static const common_regex tool_call2_regex(
            "<\\|channel\\|>(analysis|commentary)" + recipient + constraint + "?");
    ...
    auto regex_match = [](const common_regex &regex,
                          const std::string &input) -> std::optional<common_regex_match> {
    ...
    if (inputs.tools.is_array() && !inputs.tools.empty()) {
        ...
        data.grammar = build_grammar([&](const common_grammar_builder &builder) {
            ...
            foreach_function(inputs.tools, [&](const json &tool) {
                ...
                if (name == "python") {
                    args_rule = builder.add_rule(name + "-maybe-raw-args",
                                                 args_rule + " | [^{] .*");
                } else {
                    args_pattern = "\\{" + args_pattern;
                }
                auto call_rule = builder.add_rule(name + "-call",
                                                  "\"" + name + "\\n\" " + args_rule);
                first_tool_rules.push_back(call_rule);
                if (inputs.parallel_tool_calls) {
                    subsequent_tool_rules.push_back(
                            builder.add_rule(name + "-call2", "\">>>\" " + call_rule));
                }
                data.grammar_triggers.push_back({
                                                        COMMON_GRAMMAR_TRIGGER_TYPE_PATTERN_FULL,
            ...
            auto first_rule = first_tool_rules.empty() ? "" : builder.add_rule("first_tool_call",
                                                                               string_join(
                                                                                       first_tool_rules,
                                                                                       " | ")) +
                                                              " space";
            if (inputs.parallel_tool_calls) {
                auto subsequent_rule = builder.add_rule("subsequent_tool_call",
                                                        string_join(subsequent_tool_rules, " | ")) +
                                       " space";
...
common_chat_params_init_functionary_v3_1_llama_3_1(const common_chat_template &tmpl,
                                                   const struct templates_params &inputs) {
    ...
    if (!inputs.tools.is_null()) {
        ...
        data.grammar = build_grammar([&](const common_grammar_builder &builder) {
                        ...
                        for (auto it = properties.begin(); it != properties.end(); ++it) {
                            if (it.value().at("type") == "string") {
                                if (!python_code_argument_name.empty()) {
                                    throw std::runtime_error(
                                            "Multiple string arguments found in python tool");
                                }
            ...
            auto tool_call =
                    builder.add_rule("tool_call", string_join(tool_rules, " | ")) + " space";
            builder.add_rule("root",
...
static common_chat_params common_chat_params_init_hermes_2_pro(const common_chat_template &tmpl,
    ...
    if (!inputs.tools.is_null()) {
        ...
        data.grammar = build_grammar([&](const common_grammar_builder &builder) {
            ...
            auto wrappable_tool_call = builder.add_rule("wrappable_tool_call",
                                                        "( " + string_join(alt_tags, " | ") +
                                                        " ) space");
...
static common_chat_params common_chat_params_init_granite(const common_chat_template &tmpl,
    ...
    data.prompt = apply(tmpl, inputs, /* messages_override= */ std::nullopt, /* tools_override= */
                        std::nullopt, additional_context);
    ...
    if (!inputs.tools.is_null()) {
        ...
        data.grammar = build_grammar([&](const common_grammar_builder &builder) {
            ...
            auto tool_list = builder.add_rule("tool_list",
    ...
    while (auto tool_res = builder.try_find_regex(tool_call_begin_regex)) {
        ...
        if (auto func_res = builder.try_find_regex(function_regex, std::string::npos, false)) {
            ...
            if (end_func) {
                ...
                auto eaten_fragment = builder.input().substr(end_func->groups[0].begin,
                                                             end_func->groups[0].end);
                auto funlen = std::string("</function>").length();
                if (eaten_fragment.length() >= funlen &&
                    eaten_fragment.substr(0, funlen) == std::string("</function>")) {
            ...
            if (auto end_tool = builder.try_find_regex(tool_call_end_regex, std::string::npos,
                                                       false)) {
...
static common_chat_params common_chat_templates_apply_jinja(
    ...
    params.messages = common_chat_msgs_to_json_oaicompat<json>(inputs.messages, /* concat_text= */
                                                               !tmpl.original_caps().requires_typed_content);
    ...
    if (src.find("message['prefix'] is defined and message['prefix'] and thinking") !=
        std::string::npos &&
    ...
    if (src.find("elif thinking") != std::string::npos &&
...
static common_chat_params common_chat_templates_apply_legacy(
    ...
    int32_t res = llama_chat_apply_template(src.c_str(), chat.data(), chat.size(),
                                            inputs.add_generation_prompt, buf.data(), buf.size());
    ...
    if ((size_t) res > buf.size()) {
        buf.resize(res);
        res = llama_chat_apply_template(src.c_str(), chat.data(), chat.size(),
                                        inputs.add_generation_prompt, buf.data(), buf.size());
    }
...
common_chat_msg
common_chat_parse(const std::string &input, bool is_partial, const common_chat_syntax &syntax) {
