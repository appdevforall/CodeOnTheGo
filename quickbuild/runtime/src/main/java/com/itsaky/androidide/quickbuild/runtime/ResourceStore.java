...
final class ResourceStore {
	 ...
	 * @param generation
	 *            the payload generation, which names the directory so generations never mix
	 * @param cacheRoot
	 *            the app's cache directory, the parent of the per-generation directories
	 * @throws IOException
	 *             on a read, extraction, or path-traversal failure; the override is left pointing at the previous generation
	 */
	 ...
	 *
	 * @param resources
	 *            the newly created activity or context Resources to attach to; null is ignored. Call before the activity inflates anything, or it resolves against the old table.
	 */
	 ...
	 *
	 * @param path
	 *            the asset path relative to the assets root, as the app would pass to AssetManager; null yields null
	 * @return the extracted file, or null when no deploy changed that asset, the file is missing, or the path would escape the override directory. See the class doc for what this path does and does not cover.
	 */
