 ...
 *
 * The fields below are the schema: this class is its only reader, and the host writes it from {@code PayloadDeployer.metadata}. Unknown fields are ignored so the host can extend the schema without breaking installed proxy apps.
 */
final class DeployMetadata {
	 ...
	 *
	 * @param entryActivity
	 *            user activity class to launch when none is alive; null when unknown
	 * @param changedAssets
	 *            relative asset paths in this payload; null becomes the empty list
	 * @param reason
	 *            the deploy reason; null becomes {@link #REASON_UNKNOWN}
	 */
	 ...
	 *            user activity class to launch when none is alive; null when unknown
	 * @param changedAssets
	 *            relative asset paths in this payload; null becomes the empty list, and a non-null list is wrapped unmodifiable rather than copied
	 * @param reason
	 *            the deploy reason; null becomes {@link #REASON_UNKNOWN}
	 * @param restart
