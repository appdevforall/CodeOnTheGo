...
- **Two library components are excluded by name**, kept under their real manifest name: `androidx.startup.InitializationProvider` (resolves its own component by name at runtime) and `androidx.profileinstaller.ProfileInstallReceiver` (absent from some proxy compile classpaths — and before compilation, absence is indistinguishable from a project-owned class). Compose's `PreviewActivity` and Room's `MultiInstanceInvalidationService` used to be listed here too; both are now caught by the `final` rule above, from their own class bytes. Impact: none in normal use — neither is ever recompiled by the daemon, so proxying buys nothing. Status: fixed. Rationale per entry: [`docs/component-proxying-design.md`](docs/component-proxying-design.md) section 2.
