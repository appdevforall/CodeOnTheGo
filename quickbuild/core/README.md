...
- **Every bundled template that produces a runnable app live-reloads, in both languages where the template offers a choice: 7 of 8 app templates.** The 8th is No Activity, which has no Activity to launch, so it is not a runnable app in the first place. (`assets/core.cgt` ships 9 templates; the 9th is Plugin, which emits a `.cgp` rather than an app. 7 of the 9 offer Kotlin and Java, and both were walked.) `[measured on a56, 2026-07-23 sweep - run output not retained, so this needs a re-run before it ships as a claim]`. Two caveats from that walk:
