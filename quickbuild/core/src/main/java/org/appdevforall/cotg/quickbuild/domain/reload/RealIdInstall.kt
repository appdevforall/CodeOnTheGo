 ...
 * `.quickbuild` suffix - so installing one overwrites the other. Which build occupies it is read
 * statelessly from the installed package's `android:appComponentFactory`: matching
 * [QUICK_BUILD_APP_COMPONENT_FACTORY] means a Quick Build proxy app, anything else does not.
 */
object RealIdInstall {
	 ...
	 * @return the named refusal; the host owns its wording.
	 */
	fun refusalMessage(realApplicationId: String): QuickBuildMessage = QuickBuildMessage.ForeignAppInstalled(realApplicationId)
}
