...
interface DeploySender {
	 ...
	 * @param metadataJson entry activity and restart flag; see `PayloadDeployer.metadata`
	...
	override suspend fun deploy(
		...
		return withTimeoutOrNull(timeoutMillis) {
			coroutineScope {
				...
				try {
					// Hand the fds, not the bytes: the kernel dups them across the process
					// boundary, so the running app loads straight from disk with no copy. The
					// nested `use` closes our ends once the call returns; the proxy keeps its dups.
