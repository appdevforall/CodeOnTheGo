...
| [`DeployPolicy.kt`](DeployPolicy.kt) | Decides restart vs recreate by walking each restart-sensitive component's supertype closure against the changed classes; defines `DeployDecision`. |
...
| [`ClassHeader.kt`](ClassHeader.kt) | Parses a class file's name/superclass/interfaces via a constant-pool walk, to keep the supertype index current. |
