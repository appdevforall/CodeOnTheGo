...
/**
 * Gets one build's artifacts into the running proxy app and reports whether they are live.
 *
 * Owns everything downstream of the deploy decision: hot swap versus process restart, the
 * relaunch and reconnect checks, the retry when no app is connected, and the [DeployResult] to
 * [BuildOutcome] mapping. Generations are allocated here, so a build that never deploys never
 * burns one. Call only on the session dispatcher.
 */
internal class PayloadDeployer(
	 ...
	 * would happen rather than captured up front, because a tap can promote a build already in
	 * flight. Starting an activity always takes the screen, so this separates a deploy that may
	...
	/**
	 * Where a confirmed deploy's bytes are retained for the reconnect re-send
	 * (concurrency.md rules 3-4). Null retains nothing, which only costs the fallback:
	 * every below-deployed reconnect then repairs by forced rebuild.
	 */
	private val retention: RetainedPayloadStore? = null,
) {
	/**
	 ...
	 * @param loopStartedAt t0 of the save-to-live loop, which the reported duration is measured
	 *   from - the same span the timeline totals, so the two numbers cannot disagree
	...
	suspend fun deploy(
		...
		when (decision) {
			DeployDecision.Recreate -> {
				deployPayload(generations.next(), dexFile, arscFile, assets, loopStartedAt, recorder)
			}

			is DeployDecision.Restart -> {
				deployRestart(decision, dexFile, arscFile, assets, loopStartedAt, recorder)
			}
	...
	/**
	 * Restart path: deploy with restart metadata, wait for the runtime to persist and
	 * exit, relaunch it, then check which generation came back.
	 *
	 ...
	 *
	 * [launched] separates "nobody has opened your app" from "your app will not stay up": it is
	 * true only when a launch really started the app and it still did not come back, which is
	 * the evidence a repeat escalates into the cannot-stay-up dialog. A save never launches, and
	 * a launch that failed to start never ran, so neither counts.
	 *
	 ...
	 * runtime's `DeployMetadata` parses it. Adding a field here needs a matching read there - and
	 * a field the runtime does not read is bytes crossing a binder for nobody, which is why this
	 * is exactly the two keys it reads.
	 *
	 ...
	 */
	private fun metadata(restart: Boolean): String =
		JsonObject()
			.apply {
	 ...
	 *   [RecoveredDeploy.launched]); deliberately without a default, since inferring it from
	 *   [userInitiated] would flag every tap that never got as far as launching.
