 ...
 * untranslated into an IDE that has a dozen locales. Each case names a situation and carries
 * only the values the wording needs; the host maps it to a string resource.
 *
 * [Literal] is the deliberate exception - text that is already final because nothing can
 * translate it (a PackageManager verdict, an exception message) or because the host resolved
 * it from its own resources before handing it over.
 */
sealed interface QuickBuildMessage {
	/**
	 * Final text, passed through untouched.
	 *
	 * @property text either host-resolved copy or an opaque detail from Android or a thrown
	 *   exception. Never a sentence written in this module - that is what the named cases are
	 *   for.
	 */
	data class Literal(
		val text: String,
