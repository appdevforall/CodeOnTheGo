		...
		withEvents { events ->
			...
			when (outcome) {
				is BuildOutcome.Success -> {
					...
					// The same shape as relinkStuck, for the deploy half of the pipeline: a
					// second not-connected deploy running proves the proxy app cannot stay up
					// long enough to receive anything - a baseline that crashes at startup.
					// No edit reaches it (the fix compiles fine and has nowhere to land) and
					// the message's own "relaunch to reconnect" just restarts the crash, so
					// the only true remedy is a fresh proxy app build.
...
sealed interface OrchestratorEvent {
	 ...
	 * @property proxyAppWontStayUp true when a second not-connected deploy running proves the
	 *   proxy app cannot stay alive to receive a payload - a baseline that crashes at startup.
	 *   No edit reaches it and the deploy message's own "relaunch to reconnect" restarts the
	 *   same crash, so the host should offer Restart session. Set at most once per streak.
	 */
