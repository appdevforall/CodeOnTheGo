 ...
 *
 * Text rather than a parser because classification runs on every save, before any compile, with no
 * resolved PSI. It aims at never missing a change that could alter processor output and pays for
 * that with over-inclusiveness: string literals are kept verbatim (an `@Query("SELECT ...")` edit
 * IS processor input), only an unambiguous function/initializer body leaves the declaration
 * fingerprint, and a structural surprise returns null, which the analyzer reads as "rebaseline".
 *
...
object SourceAnnotationScanner {
	 ...
	 * @return the facts, or null when the text could not be scanned confidently (unbalanced braces,
	 *   unterminated comment or raw string), which callers read as "assume processor input
	 *   changed".
	 */
