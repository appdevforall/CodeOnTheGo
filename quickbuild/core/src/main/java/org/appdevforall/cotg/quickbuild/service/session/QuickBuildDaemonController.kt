...
/**
 * Owns the compile daemon's lifecycle: the epoch rule, respawn supersession, low-memory shrink.
 *
 * The epoch counts intentional daemon transitions - every start or shutdown the session manager
 * initiates outside the respawn path. [start] and [shutdown] deliberately do not bump it: the
 * teardown must bump synchronously before it suspends, and [respawn]'s cleanup rule counts
 * exactly one transition, which an auto-bump would break. Call only on the session dispatcher.
 */
