 ...
 * [SourceAnnotationScanner]. Everything here is derived from text - the live reload path
 * has no compiler front-end at classification time - so the scanner is deliberately
 ...
 * @property declarationFingerprint the file's declaration surface - every code line outside a
 *   function/initializer body, comment- and whitespace-normalized - so two revisions sharing one
 *   differ only inside executable bodies.
 ...
 * @property arguments the parenthesized argument text with whitespace collapsed, empty when there
 *   is no argument list, and itself processor input - Room reads `@Query("...")`'s SQL and
 *   `@ColumnInfo(name = ...)`'s column name.
 * @property useSiteTarget the Kotlin use-site target (`field` in `@field:Json`), kept separate
 *   from [name] so imports still resolve the name but part of equality, because `@get:Json` and
 *   `@field:Json` are different processor input.
 */
