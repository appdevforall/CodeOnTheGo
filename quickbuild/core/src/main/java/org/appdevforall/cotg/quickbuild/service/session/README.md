...
| [`LiveSession.kt`](LiveSession.kt) | Holds one live session's wiring (orchestrator, tracker, mutable baseline and `SessionWatch`); `adoptBaseline` moves it onto a rebuilt proxy app, replacing the watch set when the rebuild changed it, and `SwitchableExecutor` swaps the executor without replacing the orchestrator. |
