...
sealed interface BuildRoute {
	...
	/** Code changed, no resources: incremental compile, then d8 over the whole class tree. */
	 ...
	 * the classifier; only [org.appdevforall.cotg.quickbuild.domain.reload.LiveReloadOrchestrator.onWarmCompileRequested] constructs it.
	 */
 ...
 * live service or provider still holds ([org.appdevforall.cotg.quickbuild.domain.session.QuickBuildNotice.STALE_COMPONENT_HELPERS]).
 *
 * A forced [BuildRoute.NoOp] recompiles the whole module, so it counts. Says nothing about
 * whether anything DEPLOYED - a [BuildRoute.WarmCompile] recompiles everything and deploys
 * nothing, so a caller reasoning about the running app has to exclude it separately.
 */
...
enum class InvalidationReason {
	 ...
	 * (e.g. a java-resource under src/), or cannot deliver on this device - an asset below API
	 * 30, where the runtime has no `ResourcesLoader` to serve the deployed payload from.
	 */
	...
	/**
	 * The same infrastructure failure twice running, for something no edit can clear (a relink
	 * that cannot resolve the baseline's library resource snapshot), so only a fresh baseline can
	 * absorb the pending set. Reported at most once per baseline, so a rebuild that fails leaves
	 * plain build failures. Never raised for a compile error - see
	 ...
	 *
	 * The prompt may never even appear: Android defers the PENDING_USER_ACTION broadcast until
	 * CoGo is foregrounded, and the dialog-owning subscriber is EventBus lifecycle-bound, so the
	 * deferred delivery can land before it re-registers. The next Quick Build tap or return to
	 * the foreground re-runs the rebuild and re-prompts.
	 */
