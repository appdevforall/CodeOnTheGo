...
| `ProxyAppBuildRunner.kt` (lands with the session-orchestration PR) | Runs a provision or rebuild as a stateless verdict - disk guard, build, scratch tree, deploy session, daemon start - returning a result the manager dispatches on. |
