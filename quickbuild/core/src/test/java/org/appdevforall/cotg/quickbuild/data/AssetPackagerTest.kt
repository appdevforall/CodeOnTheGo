...
class AssetPackagerTest {
	...
	@Test
	fun `a path that climbs out of the asset root is not an asset`() {
		// Raw text, the escape passes a startsWith check against the root and would name a
		// zip entry the runtime unpacks outside its asset directory.
		val escaping = File(assetsRoot, "sub/../../../evil.txt")

		assertThat(packager.relativeAssetPath(escaping, listOf(assetsRoot))).isNull()
	}

	@Test
	fun `a path that climbs but stays inside keeps its resolved name`() {
		val inside = File(assetsRoot, "sub/../data/levels.json")

		assertThat(packager.relativeAssetPath(inside, listOf(assetsRoot)))
			.isEqualTo("data/levels.json")
	}

	@Test
	fun `an escaping path is packaged as no asset at all`() {
		val escaping = File(assetsRoot, "sub/../../../evil.txt")
		val out = File(tempDir, "payload.zip")

		assertThat(packager.packageAssets(listOf(escaping), listOf(assetsRoot), out)).isNull()
		assertThat(out.exists()).isFalse()
	}
}
