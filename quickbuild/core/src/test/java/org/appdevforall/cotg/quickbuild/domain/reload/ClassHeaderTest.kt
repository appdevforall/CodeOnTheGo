...
class ClassHeaderTest {
	...
	fun `parses name, superclass and interfaces of a real nested class`() {
		...
		assertThat(header!!.className)
			.isEqualTo("org.appdevforall.cotg.quickbuild.domain.reload.ClassHeaderTest\$Sample")
		assertThat(header.superClassName)
			.isEqualTo("org.appdevforall.cotg.quickbuild.domain.reload.ClassHeaderTest\$Base")
	...
	fun `constant-pool entries with two slots do not derail the walk`() {
		...
		assertThat(header).isNotNull()
		assertThat(header!!.className)
			.isEqualTo("org.appdevforall.cotg.quickbuild.domain.reload.ConstantsFixture")
	}
