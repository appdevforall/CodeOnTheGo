 ...
 *
 * Its only production caller gates the stale-classes guard
 * (`QuickBuildSessionManager`: `if (event.route is BuildRoute.WarmCompile ||
 * !event.route.recompilesCode) return`), so a route landing on the wrong side silently
 * either skips a guard that should run or runs one that should not. Reached only through
 * that guard before, which left most routes unexercised.
 */
