...
/**
 * A minimal Room + Hilt app materialized on disk, so the analyzer runs against real files.
 *
 * Covers the shapes that make annotation-aware classification hard: an `@Entity` with an
 * un-annotated `@Embedded` value type, a non-annotated base class an entity inherits from, a
 * `@TypeConverters` converter, `@Dao` SQL in annotation arguments, and plain UI files that
 * must never rebaseline.
 */
