...
/**
 * Pins the debounce policy: a trailing [quietMillis] window, reset on every event, capped
 * at [maxMillis] since the batch's first event. Virtual-time tests so they are
 * deterministic and instant. Batches carry both the modified/created paths and the removed
 * ones.
 */
class ChangeCoalescingTest {
	...
	fun `a continuous stream is capped and flushes at maxMillis`() =
		runTest {
			// An event every 120 ms (< 150 quiet) for 1.8 s: the quiet timer keeps resetting,
			// so only the cap can flush. 120 rather than 100 so no event falls on maxMillis
			// itself, which would leave the batch it lands in decided by timer tie-breaking.
			val source =
				flow {
					repeat(15) { i ->
						emit(m("S$i.kt"))
			...
			val flushedAt = mutableListOf<Long>()
			source.coalesceChanges(quietMillis = 150, maxMillis = 1000).collect { batch ->
				batches.add(batch)
				flushedAt.add(testScheduler.currentTime)
			}

			// The cap - not the end of the stream - produces the first batch: it lands at
			// maxMillis carrying everything written by then, and the stragglers wait for the
			// terminal flush. Assert the two batches separately rather than as a union, so a
			// path duplicated into both (or dropped from one) cannot hide.
			assertThat(batches).hasSize(2)
			assertThat(flushedAt[0]).isEqualTo(1000L)
			assertThat(batches[0].files).containsExactlyElementsIn((0..8).map { f("S$it.kt") })
			assertThat(flushedAt[1]).isEqualTo(1800L)
			assertThat(batches[1].files).containsExactlyElementsIn((9..14).map { f("S$it.kt") })
		}
