...
class PayloadDeployerTest {
	...
	private suspend fun deployRestart(deployer: PayloadDeployer): BuildOutcome =
		deployer.deploy(restart, null, null, null, loopStartedAt = 0, recorder = recorder())
	...
	fun `restart reconnect at the deployed generation is a restarted success`() =
		runTest {
			...
			// The exit wait is bounded by the DISCONNECT timeout, not the reconnect one:
			// waiting a reconnect-sized 15s for a process that already died is 10s of dead
			// air on every service edit.
			assertThat(deploy.awaitDisconnectCalls).containsExactly(5_000L)
		}
	 ...
	 * plain save touching a Service or Receiver steals focus. Deferred, not overlooked - a
	 * restart cannot finish without the process coming back, so suppressing the relaunch needs
	 * a decision about what a half-restarted app does, not a one-line gate. Pinned so gating
	 * on [userInitiated] turns this red and whoever does it has to say so here.
	 */
	@Test
