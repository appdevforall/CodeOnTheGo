...
class ProxyAppInstallerTest {
	...
	fun `installed package with identical bytes is skipped - no dialog, no reinstall`() =
		runTest {
			...
			assertThat(outcome).isEqualTo(InstallOutcome.Installed(10123))
	...
	fun `fresh install resolves via the success broadcast and the new uid`() =
		runTest {
			...
			assertThat(result.await()).isEqualTo(InstallOutcome.Installed(10456))
		}
	...
	fun `failure broadcast surfaces the real installer message fast`() =
		runTest {
			...
			assertThat(result.await())
				.isEqualTo(InstallOutcome.Failed(QuickBuildMessage.Literal("INSTALL_FAILED_INVALID_APK")))
		}
	...
	fun `intent-fallback installs (no broadcast) complete via the lastUpdateTime poll`() =
		runTest {
			...
			assertThat(result.await()).isEqualTo(InstallOutcome.Installed(10789))
		}
	...
	fun `launch failure fails immediately`() =
		runTest {
			...
			assertThat(outcome)
				.isEqualTo(InstallOutcome.Failed(QuickBuildMessage.InstallCouldNotStart))
		}
	...
	fun `foreground timeout is ConfirmationNotGiven TIMED_OUT, never a false success`() =
		runTest {
			...
			assertThat((outcome as InstallOutcome.ConfirmationNotGiven).reason)
				.isEqualTo(InstallOutcome.ConfirmationNotGiven.Reason.TIMED_OUT)
			assertThat(outcome.message).isInstanceOf(QuickBuildMessage.ReinstallTimedOut::class.java)
		}
	...
	fun `backgrounded timeout reports the dialog was never shown - return to CoGo`() =
		runTest {
			...
			assertThat(outcome.message).isEqualTo(QuickBuildMessage.ReinstallReturnToCoGo)
		}
	...
	fun `an aborted install is ConfirmationNotGiven DECLINED, not a hard failure`() =
		runTest {
			...
			assertThat(outcome.message).isEqualTo(QuickBuildMessage.ReinstallDeclined)
		}
	...
	fun `a prompt nobody was ever shown is re-issued once inside the same budget`() =
		runTest {
			...
			// Under the window: a user still reading the dialog is left alone.
			...
			advanceTimeBy(2_000L)
			runCurrent()
			assertThat(installLaunches).containsExactly(apk, apk)

			// The second prompt is answered, well inside the 180s whole-install budget.
			packages.uid = 10123
			broadcasts.emit(InstallBroadcast(PKG, InstallBroadcast.Status.SUCCESS))
			advanceUntilIdle()
			assertThat(result.await()).isEqualTo(InstallOutcome.Installed(10123))
	...
	@Test
	fun `a declined prompt is never re-issued`() =
		runTest {
			// The user answered. Re-prompting would nag them with the dialog they just
			// dismissed.
			val result = async { installer(promptTimeoutMillis = 45_000L).ensureInstalled(apk, PKG) }
			runCurrent()
			...
			assertThat((result.await() as InstallOutcome.ConfirmationNotGiven).reason)
				.isEqualTo(InstallOutcome.ConfirmationNotGiven.Reason.DECLINED)
			assertThat(installLaunches).containsExactly(apk)
		}

	@Test
	fun `an install answered before the window is not re-prompted`() =
		runTest {
			val result = async { installer(promptTimeoutMillis = 45_000L).ensureInstalled(apk, PKG) }
			runCurrent()
	...
	@Test
	fun `two unanswered prompts still report TIMED_OUT, not a false success`() =
		runTest {
			val result =
				async {
					installer(timeoutMillis = 100_000L, promptTimeoutMillis = 40_000L)
						.ensureInstalled(apk, PKG)
				}
			advanceUntilIdle()
	...
	@Test
	fun `a backgrounded install is not re-prompted - nobody could see the second dialog either`() =
		runTest {
			confirmDialogShowable = false
			val result =
				async {
	...
	fun `a real failure broadcast is Failed, not ConfirmationNotGiven`() =
		runTest {
			...
			assertThat(result.await()).isEqualTo(InstallOutcome.Failed(QuickBuildMessage.Literal("rejected")))
		}
	...
	fun `success broadcast but unresolvable uid fails visibly after retries`() =
		runTest {
			...
			assertThat((outcome as InstallOutcome.Failed).message)
				.isInstanceOf(QuickBuildMessage.InstalledButUnresolvable::class.java)
		}
	...
	fun `uid appearing after a retry still resolves`() =
		runTest {
			...
			assertThat(result.await()).isEqualTo(InstallOutcome.Installed(10999))
		}
	...
	fun `failure broadcast without a message falls back to a generic one`() =
		runTest {
			...
			assertThat(result.await())
				.isEqualTo(InstallOutcome.Failed(QuickBuildMessage.InstallFailed))
		}
