...
class BuildStatusJsonTest {
	...
	@Test
	fun `never sends the error location to the device`() {
		// Jumping to an error is CoGo-side functionality; the runtime has no use for a
		// host-side path, so position data stays off the deploy channel entirely.
		val obj =
			parse(
				BuildStatusJson.buildFailed(
					listOf(error("Unresolved reference: foo", "/p/src/Foo.kt", 12, 5)),
		...
		assertThat(obj.keySet()).containsExactly("kind", "message")
	}

	@Test
	fun `reinstall pending is kind-only`() {
		// The copy is static and lives runtime-side with the other overlay text, so the
		// wire carries nothing but the kind.
		val obj = parse(BuildStatusJson.reinstallPending())

		assertThat(obj.get("kind").asString).isEqualTo("reinstall_pending")
		assertThat(obj.keySet()).containsExactly("kind")
	}
