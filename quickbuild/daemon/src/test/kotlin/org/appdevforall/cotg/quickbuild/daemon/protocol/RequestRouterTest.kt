...
class RequestRouterTest {
	...
	@Test
	fun `ping replies ok with the protocol version, without touching handlers`() {
		val routed = router.route(PingRequest(5))

		assertThat(routed).isInstanceOf(RequestRouter.Routed.Reply::class.java)
		assertThat(routed.response)
			.isEqualTo(DaemonResponse.ok(5, mapOf("protocolVersion" to DaemonResponse.PROTOCOL_VERSION)))
