 ...
 * real SDK. The argument-assembly and reset-guard tests below run fake or absent binaries, so a
 * host without an Android SDK must still execute them.
 */
class Aapt2LinkTest {
	@TempDir
	...
	fun `relinked apk carries file-backed resources, not just the arsc table`() {
		// A drawable XML has no useful value inside resources.arsc alone - the runtime needs the
		// actual zip entry to resolve it. Ship only the table and ANY file-backed resource (even
		// one the edit never touched, e.g. an adaptive-icon mipmap XML) fails to resolve on the
		// next activity recreate.
	...
	fun `a missing aapt2 binary fails with a message, not a throw`() {
		// Fails in the compile phase, before android.jar is ever named, so a fake jar path keeps
		// this runnable on a host with no SDK.
		writeStrings("<resources />")
		val link = Aapt2Link(File(tempDir, "no-such-aapt2"), File(tempDir, "android.jar"))
	...
	// aapt2's declaration-order type-index assignment shifts when a whole resource TYPE the real
	// proxy app build produced (e.g. a library-injected `bool`) is absent from a relink's narrower
	// res tree - the manifest, compiled once against the baseline table, then decodes its numeric
	// ids against the WRONG type. `--stable-ids` pins ids to the baseline regardless. No real
	// toolchain needed: `buildLinkArguments` is pure argument assembly, unlike `relink` itself.
	...
	// A relink of the project's own res/ alone can't resolve a resource a dependency AAR provides
	// (e.g. Material3's Theme.Material3.DayNight.NoActionBar), so the daemon feeds pre-compiled
	// library-resource units back in as `-R` overlays.
