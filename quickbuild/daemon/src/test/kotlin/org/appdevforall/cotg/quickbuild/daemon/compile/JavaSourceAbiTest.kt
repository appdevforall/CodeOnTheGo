...
class JavaSourceAbiTest {
	...
	private fun repository(dateImport: String) =
		write(
			"Repository.java",
			"""
			package demo;
	...
	@Test
	fun `swapping an import for a same-simple-name type moves the fingerprint`() {
		// The signature text does not move - it still reads `Date created()` - but the type a
		// Kotlin caller links against does. Miss this and changedTypeNames comes back empty,
		// no Kotlin file recompiles, and the un-recompiled caller keeps a checkcast against
		// the old class: ClassCastException in the running app.
		val before = fingerprintOf(repository("java.util.Date"))

		val after = fingerprintOf(repository("java.sql.Date"))

		assertThat(after).isNotEqualTo(before)
	}

	@Test
	fun `an import swap names the declaring type as changed, forcing a Kotlin recompile`() {
		val previous = JavaSourceAbi.snapshot(listOf(repository("java.util.Date")))!!
		val current = JavaSourceAbi.snapshot(listOf(repository("java.sql.Date")))!!

		assertThat(JavaSourceAbi.changedTypeNames(previous, current)).containsExactly("Repository")
	}

	@Test
	fun `reordering imports leaves the fingerprint unchanged`() {
		// Imports are hashed sorted, so a formatter's reorder must not cost a full Kotlin
		// recompile - only a change to the set of imported types does.
		val before =
			fingerprintOf(
				write(
					"Ordered.java",
					...
					public class Ordered {
						public List<Map<String, String>> rows() { return null; }
					}
					""",
				),
			)
