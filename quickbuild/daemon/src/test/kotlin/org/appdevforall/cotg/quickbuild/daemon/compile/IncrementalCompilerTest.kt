...
class IncrementalCompilerTest {
	...
	fun `editing one file recompiles incrementally, not a full rebuild`() {
		...
		// The seed compile above legitimately recompiles everything; only the edit's own log
		// says whether THIS compile was incremental.
		compileLog.clear()
		val result = compiler.compile(sources, changedFiles = listOf(greeter))

		assertThat(result).isInstanceOf(IncrementalCompiler.Result.Success::class.java)
		val log = compileLog.joinToString("\n")
		...
		val iterationLines = compileLog.filter { it.contains("compile iteration") }
	...
	@Test
	fun `a removed kotlin source has its output deleted`() {
		val orphan = writeSource("Orphan.kt", "package demo\n\nclass Orphan")
		val sources = listOf(greeterKt(), mainKt(), orphan)
		val compiler = compiler()
		val first = compiler.compile(sources, changedFiles = sources)
		assertThat(first).isInstanceOf(IncrementalCompiler.Result.Success::class.java)
		val classesDir = (first as IncrementalCompiler.Result.Success).classesDir
		assertThat(File(classesDir, "demo/Orphan.class").isFile).isTrue()

		// Orphan.kt is deleted: gone from allSources AND passed as a removal. Threaded into
		// SourcesChanges.Known's removed slot, the engine must delete its stale output so a
		// deleted class can't survive into the dex.
		assertThat(orphan.delete()).isTrue()
		val result =
			compiler.compile(
				listOf(greeterKt(), mainKt()),
				changedFiles = emptyList(),
				removedFiles = listOf(orphan),
			)

		assertThat(result).isInstanceOf(IncrementalCompiler.Result.Success::class.java)
		assertThat(File(classesDir, "demo/Orphan.class").exists()).isFalse()
	}

	@Test
	fun `a removed java source has its class deleted before it can reach the dex`() {
		// javac never deletes outputs for sources it's no longer handed, so the daemon must
		// delete a removed .java's .class explicitly. The path mirrors its package under a
		// main/java root, exactly as the enforced project layout does.
		val widget =
	...
	fun `a stale java class that cannot be deleted fails the compile instead of riding into the dex`() {
		...
		// exactly the "stubborn stale output" this guard exists for.
		val widget =
			File(srcDir, "main/java/demo/Widget.java").apply {
				parentFile!!.mkdirs()
				writeText("package demo;\n\npublic class Widget { public int v() { return 1; } }")
	...
	fun `composable edit recompiles incrementally with the plugin active`() {
		...
		// Read the edit's own log, not the seed's.
		compileLog.clear()
	...
	fun `kotlin source resolves a same-module java class it calls`() {
		...
		// a sibling .java file that isn't precompiled onto the classpath yet, and the
		// baseline compile fails outright with "Unresolved reference".
		val javaSource =
			writeSource(
				"JavaCalculator.java",
				"""
	...
	private fun labelsKt(suffix: String = "MY_LABEL_V1") =
		writeSource(
			"Labels.kt",
			"""
			package demo
			...
			object Labels {
				inline fun label(prefix: String): String = prefix + "$suffix"
			}
			""".trimIndent(),
		)

	private fun labelUserKt() =
		writeSource(
			"LabelUser.kt",
			"""
			package demo
			...
			class LabelUser {
				fun render(): String = Labels.label("prefix: ")
			}
			""".trimIndent(),
		)

	@Test
	fun `an inline function's body edit recompiles its unedited caller`() {
		// An inline function's BODY is part of its ABI - it is copied into every call site -
		// so an edit that moves no signature must still recompile untouched callers. That's
		// a different invalidation rule from the signature-change cases above, and Kotlin's
		// IC has historically got it wrong: the caller then keeps running the old inlined
		// body while its source says otherwise.
		val labels = labelsKt()
		val sources = listOf(labels, labelUserKt())
		val compiler = compiler()
		val baseline = compiler.compile(sources, changedFiles = sources)
		assertThat(baseline).isInstanceOf(IncrementalCompiler.Result.Success::class.java)
		val classesDir = (baseline as IncrementalCompiler.Result.Success).classesDir
		val before = File(classesDir, "demo/LabelUser.class").readBytes()
		// Premise check: the literal only lands in the CALLER's constant pool if the body
		// really was inlined. Without this the edit assertion below could pass vacuously.
		assertThat(String(before, Charsets.ISO_8859_1)).contains("MY_LABEL_V1")

		labelsKt(suffix = "MY_LABEL_V2")
		// The seed compile legitimately compiles everything; only the edit's own log says
		// whether THIS compile recompiled the caller by invalidation or by falling back.
		compileLog.clear()
		val result = compiler.compile(sources, changedFiles = listOf(labels))

		assertThat(result).isInstanceOf(IncrementalCompiler.Result.Success::class.java)
		val after = File(classesDir, "demo/LabelUser.class").readBytes()
		assertThat(after).isNotEqualTo(before)
		// The untouched caller's own bytecode must now carry the new body, and not the old.
		assertThat(String(after, Charsets.ISO_8859_1)).contains("MY_LABEL_V2")
		assertThat(String(after, Charsets.ISO_8859_1)).doesNotContain("MY_LABEL_V1")
		// The caller is reported as changed, which is what feeds CoGo's restart decision.
		assertThat((result as IncrementalCompiler.Result.Success).changedClassFiles)
			.contains("demo/LabelUser.class")
		// A full-rebuild fallback would satisfy everything above for the wrong reason, so
		// require that the caller was reached by invalidation.
		val log = compileLog.joinToString("\n")
		assertThat(log).doesNotContainMatch("(?i)non-incremental")
		assertThat(log).doesNotContain("CLASSPATH_SNAPSHOT_NOT_FOUND")
		assertThat(log).doesNotContain("UNKNOWN_CHANGES_IN_GRADLE_INPUTS")
