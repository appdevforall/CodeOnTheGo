...
class DaemonServiceOpsTest {
	...
	@Test
	fun `a configure that throws does not release the live session's tools`() {
		// A session's tools are released only once its replacement exists. Releasing first
		// stranded the still-installed session with a closed r8 class loader and a finished
		// compilation project - and the damage is LATENT, because a closed URLClassLoader still
		// serves the classes it already loaded, so it surfaces later as a NoClassDefFoundError
	...
	@Test
	fun `shutdown releases the session and is safe to repeat`() {
		configure()

		service.shutdown()
		service.shutdown()

		val response = service.compile(CompileRequest(2, emptyList(), emptyList()))
		assertThat(response.ok).isFalse()
		assertThat(response.diagnostics.single().message).contains("not configured")
	}
