...
class PluginLoader(
	...
	fun getPluginMetadata(): PluginManifest? {
		try {
			val packageInfo =
				pluginPackageInfo ?: context.packageManager.getPackageArchiveInfo(
					pluginApk.absolutePath,
					PackageManager.GET_META_DATA,
				) ?: return null
			...
			val vcsRevision = normalizeProvenanceValue(metaData.getString("plugin.vcs_revision"))
			val buildTimestamp = normalizeProvenanceValue(metaData.getString("plugin.build_timestamp"))
