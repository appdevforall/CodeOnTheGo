...
private fun sha256WithRsaAlgId(): ByteArray =
    derSeq(SHA256_WITH_RSA_OID + byteArrayOf(0x05, 0x00))   // NULL
...
private fun derContextTagged(tag: Int, content: ByteArray): ByteArray =
    tlv(0xA0 or tag, content)
...
private fun derTime(date: Date): ByteArray {
    ...
    return if (cal.get(Calendar.YEAR) >= 2050) {
        ...
        val s = "%02d%02d%02d%02d%02d%02dZ".format(
            cal.get(Calendar.YEAR) % 100, cal.get(Calendar.MONTH) + 1, cal.get(Calendar.DAY_OF_MONTH),
            cal.get(Calendar.HOUR_OF_DAY), cal.get(Calendar.MINUTE), cal.get(Calendar.SECOND)
        )
...
private fun tlv(tag: Int, content: ByteArray): ByteArray {
    ...
    when {
        ...
        else -> { out.write(0x82); out.write(len ushr 8); out.write(len and 0xFF) }
    }
    out.write(content)
