...
class SqliteMmapConfiguratorTest {
	...
	/** Runs [SqliteMmapConfigurator.configureMmap] against a database whose `rawQuery()`
	 * throws [failure], returning whatever escaped -- `null` when nothing did. */
	private fun configureMmapFailingWith(failure: Throwable): Throwable? {
		...
		return runCatching { SqliteMmapConfigurator.configureMmap(db) }.exceptionOrNull()
	}
}
